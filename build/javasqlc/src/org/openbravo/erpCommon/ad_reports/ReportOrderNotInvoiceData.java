...
static Logger log4j = Logger.getLogger(ReportOrderNotInvoiceData.class);
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("orgname"))
      ...
      return address;
    else if (fieldName.equalsIgnoreCase("maxdate"))
      return maxdate;
    else if (fieldName.equalsIgnoreCase("username"))
      return username;
    else if (fieldName.equalsIgnoreCase("description"))
      return description;
    else if (fieldName.equalsIgnoreCase("statusinv"))
      return statusinv;
  ...
  public static ReportOrderNotInvoiceData[] select(ConnectionProvider connectionProvider, String cCurrencyConv, String adUserClient, String adUserOrg, String cBpartnerId, String cOrgId, String invoiceRule, String dateFrom, String dateTo, String deliveryDateFrom, String deliveryDateTo, String adLanguage, String invoicePercent)    throws ServletException {
    return select(connectionProvider, cCurrencyConv, adUserClient, adUserOrg, cBpartnerId, cOrgId, invoiceRule, dateFrom, dateTo, deliveryDateFrom, deliveryDateTo, adLanguage, invoicePercent, 0, 0);
  }

  public static ReportOrderNotInvoiceData[] select(ConnectionProvider connectionProvider, String cCurrencyConv, String adUserClient, String adUserOrg, String cBpartnerId, String cOrgId, String invoiceRule, String dateFrom, String dateTo, String deliveryDateFrom, String deliveryDateTo, String adLanguage, String invoicePercent, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "     SELECT ORGNAME, C_BPARTNER_ID, BPARTNERNAME, C_ORDER_ID, DOCUMENTNO," +
      "       ADDRESS, MAXDATE, USERNAME, DESCRIPTION, STATUSINV," +
      "       DATEORDERED, GRANDTOTAL, CONVGRANDTOTAL, INVOICERULE, LINE, PRODUCT, PRODUCTVALUE, PRICE, CONVPRICE," +
      ...
      "             loc.name AS ADDRESS, " +
      "      	     to_char(C.DatePromised, 'DD-MM-YYYY') AS MAXDATE, 	" +
      ...
      "             from c_orderline ol   where ol.c_order_id=C.c_order_id and  ol.c_order_discount_id is null), 0)) AS STATUSINV " +
      "           FROM c_order C JOIN C_Orderline CL on CL.C_Order_ID = C.C_Order_ID " +
      "             JOIN AD_REF_LIST ON C.INVOICERULE = AD_REF_LIST.VALUE AND AD_REF_LIST.AD_REFERENCE_ID = '150' " +
      "             LEFT JOIN C_BPartner_Location loc ON loc.C_BPartner_Location_id = C.C_BPartner_Location_id " +
      "	     LEFT JOIN AD_USER usr ON usr.ad_user_id = C.SalesRep_ID " +
    ...
    strSql = strSql + ((deliveryDateFrom==null || deliveryDateFrom.equals(""))?"":"  AND C.DatePromised >= to_date(?)  ");
    strSql = strSql + ((deliveryDateTo==null || deliveryDateTo.equals(""))?"":"  AND C.DatePromised < to_date(?)  ");
    strSql = strSql + 
      "        )" +
      "        SELECT AD_ORG.NAME AS ORGNAME, C_BPARTNER.C_BPARTNER_ID, C_BPARTNER.NAME AS BPARTNERNAME," +
      "          pendingOrders.ADDRESS, pendingOrders.MAXDATE, pendingOrders.USERNAME, pendingOrders.DESCRIPTION, pendingOrders.STATUSINV, " +
      ...
      "          COALESCE(P.NAME, P.DESCRIPTION) AS PRODUCT, P.VALUE AS PRODUCTVALUE, pendingOrders.PRICEACTUAL AS PRICE," +
      ...
      "          AND pendingorders.STATUSINV <> 100" +
      "          AND (COALESCE(?, 'V') = 'V' OR (COALESCE(?, 'V') = 'C' AND pendingorders.STATUSINV = 0) OR (COALESCE(?, 'V') = 'U' AND pendingorders.STATUSINV BETWEEN 1 AND 99))" +
      "       UNION ALL" +
      "         SELECT MAX(AD_ORG.NAME) AS ORGNAME, MAX(C_BPARTNER.C_BPARTNER_ID) AS C_BPARTNER_ID, MAX(C_BPARTNER.NAME) AS BPARTNERNAME," +
      "           MAX(pendingOrders.ADDRESS) AS ADDRESS, MAX(pendingOrders.MAXDATE) AS MAXDATE, MAX(pendingOrders.USERNAME) AS USERNAME, MAX(pendingOrders.DESCRIPTION) AS DESCRIPTION, MAX(pendingOrders.STATUSINV) AS STATUSINV," +
      ...
      "           NULL AS LINE, MAX(C_TAX.NAME) AS PRODUCT, NULL AS PRODUCTVALUE, NULL AS PRICE, NULL AS CONVPRICE, NULL AS QTYORDERED, NULL AS UOMSYMBOL," +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      if (deliveryDateFrom != null && !(deliveryDateFrom.equals(""))) {
        iParameter++; UtilSql.setValue(st, iParameter, 12, null, deliveryDateFrom);
      }
      if (deliveryDateTo != null && !(deliveryDateTo.equals(""))) {
        iParameter++; UtilSql.setValue(st, iParameter, 12, null, deliveryDateTo);
      }
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, cCurrencyConv);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, cCurrencyConv);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, cCurrencyConv);
      ...
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, invoicePercent);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, invoicePercent);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, invoicePercent);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, cCurrencyConv);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, cCurrencyConv);
      ...
      while(continueResult && result.next()) {
        ...
        objectReportOrderNotInvoiceData.address = UtilSql.getValue(result, "address");
        objectReportOrderNotInvoiceData.maxdate = UtilSql.getValue(result, "maxdate");
        objectReportOrderNotInvoiceData.username = UtilSql.getValue(result, "username");
        objectReportOrderNotInvoiceData.description = UtilSql.getValue(result, "description");
        objectReportOrderNotInvoiceData.statusinv = UtilSql.getValue(result, "statusinv");
        ...
        objectReportOrderNotInvoiceData.productvalue = UtilSql.getValue(result, "productvalue");
