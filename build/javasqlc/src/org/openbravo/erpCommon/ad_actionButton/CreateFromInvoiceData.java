...
static Logger log4j = Logger.getLogger(CreateFromInvoiceData.class);
  ...
  public static CreateFromInvoiceData[] selectFromPOSOTrxCombo(ConnectionProvider connectionProvider, String language, String adUserClient, String adOrgClient, String cBpartnerId, String isTaxIncluded, String currencyId, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "           ol.qtyordered - COALESCE(il.qty,0) qty" +
      "          FROM c_orderline ol" +
      ...
      "          WHERE sb.ad_table_id = '259' AND ol.qtyordered - COALESCE(il.qty,0) > 0" +
      "         ) sb" +
    ...
    strSql = strSql + 
      ...
      "        AND o.c_currency_id = ?" +
      "        AND o.C_Order_Id = ic.C_Order_Id" +
      "        AND sb.c_order_id = o.c_order_id" +
  ...
  public static CreateFromInvoiceData[] selectFromPOUpdateSOTrx(Connection conn, ConnectionProvider connectionProvider, String adLanguage, String cOrderlineId, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT (CASE WHEN il.M_INOUTLINE_ID IS NULL THEN ol.qty ELSE il.qty END) AS ID, " +
      ...
      "        (CASE WHEN il.M_INOUTLINE_ID IS NULL THEN l.quantityOrder*C_DIVIDE((l.QtyOrdered-COALESCE(l.QTYINVOICED ,0)),(l.QtyOrdered)) ELSE l.quantityOrder END) AS quantityOrder, " +
      ...
      "        FROM C_ORDERLINE l" +
      "         LEFT JOIN (" +
      "          SELECT iol.m_inoutline_id, il.c_orderline_id, iol.M_Product_UOM_ID, iol.M_ATTRIBUTESETINSTANCE_ID, iol.movementqty," +
      "           iol.movementqty - COALESCE(il.qty,0) qty" +
      "          FROM m_inoutline iol" +
      "           JOIN sinvmin_square_bill sb ON sb.m_inoutline_id = iol.m_inoutline_id" +
      "           LEFT JOIN (" +
      "            SELECT m_inoutline_id, c_orderline_id, SUM(qty) qty" +
      "            FROM sinvmin_square_bill" +
      "            WHERE ad_table_id = '318' AND m_inoutline_id IS NOT NULL" +
      "            GROUP BY m_inoutline_id, c_orderline_id" +
      "           ) il ON il.m_inoutline_id = iol.m_inoutline_id" +
      "          WHERE sb.ad_table_id = '319' AND iol.movementqty - COALESCE(il.qty,0) > 0" +
      "         ) il on l.C_OrderLine_ID = il.C_OrderLine_ID," +
    ...
    strSql = strSql + 
      ...
      "        l.QTYINVOICED, il.M_InOutLine_ID, il.MovementQty, l.Description, l.quantityOrder, " +
      ...
      "        ol.qty, il.qty" +
      "        HAVING ( (l.explode='Y') OR (ol.qty > 0)) " +
      "        ORDER BY l.Line";
  ...
  public static CreateFromInvoiceData[] selectFromShipmentSOTrxCombo(ConnectionProvider connectionProvider, String adLanguage, String adUserClient, String adOrgClient, String cBpartnerId, String isTaxIncluded, String currencyId, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        AND EXISTS (" +
      "         SELECT iol.m_inout_id, iol.m_inoutline_id, iol.movementqty, COALESCE(sbil.qty,0) qtyinvoiced, iol.movementqty - COALESCE(sbil.qty,0) qty" +
      "         FROM m_inoutline iol" +
      "          JOIN sinvmin_square_bill sb ON sb.m_inoutline_id = iol.m_inoutline_id" +
      "          LEFT JOIN (" +
      "           SELECT m_inoutline_id, SUM(qty) qty" +
      "           FROM sinvmin_square_bill" +
      "           WHERE ad_table_id = '318' AND m_inoutline_id IS NOT NULL" +
      "           GROUP BY m_inoutline_id" +
      "          ) sbil ON sbil.m_inoutline_id = iol.m_inoutline_id" +
      "          LEFT JOIN c_orderline ol ON ol.c_orderline_id = iol.c_orderline_id" +
      "          LEFT JOIN c_order o ON o.c_order_id = ol.c_order_id" +
      "          LEFT JOIN m_pricelist pl ON pl.m_pricelist_id = o.m_pricelist_id" +
      "         WHERE sb.ad_table_id = '319' AND sb.c_invoiceline_id IS NULL" +
      "          AND iol.movementqty - COALESCE(sbil.qty,0) <> 0" +
      "          AND (ol.c_orderline_id IS NULL OR pl.istaxincluded = ?)" +
      "          AND (ol.c_orderline_id IS NULL OR o.c_currency_id = ?)" +
  ...
  public static CreateFromInvoiceData[] selectFromShipmentTrlSOTrx(ConnectionProvider connectionProvider, String adLanguage, String adUserClient, String adOrgClient, String mInoutId, String isTaxIncluded, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      "        SELECT s.M_InOut_ID AS ID, Ad_Column_Identifier(to_char('M_InOut'), to_char(s.M_InOut_ID), to_char(?)) AS NAME," +
      "        s.DocumentNo AS NAMESHIPMENT," +
      "        sb.qty AS QTY, l.C_UOM_ID,COALESCE(uomt.UOMSymbol, uom.UOMSymbol) AS UOMSymbol, " +
      "        l.M_Product_ID,Ad_Column_Identifier(to_char('M_Product'), to_char(l.m_product_id), to_char(?))||' '|| (CASE WHEN ma.serno IS NOT NULL THEN TO_CHAR('('||' '||ma.serno||' '||')') ELSE TO_CHAR('')  END) AS RELATION_NAME, l.M_InOutLine_ID,l.Line, l.M_InOutLine_ID as C_OrderLine_ID, " +
      "        COALESCE(l.A_Asset_ID, s.A_Asset_ID) AS A_Asset_ID, COALESCE(l.C_Project_ID, s.C_Project_ID) AS C_Project_ID, " +
      "        COALESCE(l.C_Costcenter_ID, s.C_Costcenter_ID) AS C_Costcenter_ID," +
      "        COALESCE(l.User1_ID, s.User1_ID) AS User1_ID, COALESCE(l.User2_ID, s.User2_ID) AS User2_ID," +
      ...
      "        M_INOUT s,    M_PRODUCT p," +
      "         (" +
      "          SELECT iol.m_inoutline_id, iol.movementqty qtydelivered," +
      "           iol.movementqty - COALESCE(il.qty,0) qty" +
      "          FROM m_inoutline iol" +
      "           JOIN sinvmin_square_bill sb ON sb.m_inoutline_id = iol.m_inoutline_id" +
      "           LEFT JOIN (" +
      ...
      "         ) sb" +
      "        WHERE s.M_INOUT_ID = l.M_INOUT_ID" +
      "        AND sb.m_inoutline_id = l.m_inoutline_id" +
      "        AND s.IsSOTrx='Y' " +
      "        AND s.DocStatus IN ('CL','CO') " +
      "        AND s.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
 ...
 */
  ...
  public static int deleteFromInvoiceDuplicateLines(Connection conn, ConnectionProvider connectionProvider, String cInvoiceId, String mProductId, String cOrderLineId, String mInOutLineId)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "      DELETE FROM C_InvoiceLine" +
      "      WHERE C_Invoice_ID = ?" +
      "      AND M_Product_ID = ?" +
      "      AND (C_OrderLine_ID = ? OR M_InOutLine_ID = ?)";
