...
package ec.com.sidesoft.production.quality.ad_actionButton;
...
static Logger log4j = Logger.getLogger(EditCCPMeasureValuesValuesData.class);
  ...
  public static EditCCPMeasureValuesValuesData[] select(ConnectionProvider connectionProvider, String maMeasureGroupId, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        from (" +
      "            select case when (t.em_spquly_takenumber is not null OR coalesce(t.em_spquly_takenumber,0) <> 0) then TO_CHAR(coalesce(t.em_spquly_takenumber,0)) else TO_CHAR(t.MEASUREHOUR, 'HH24:MI') end as MEASUREHOUR, v.seqno, c.name || (CASE c.CRITICAL WHEN 'Y' THEN ' (PC)' ELSE '' END) AS name, COALESCE((CASE v.VALUETYPE WHEN 'N' THEN TO_CHAR(v_number) WHEN " +
      "                'C' THEN 'Y' ELSE TO_CHAR(v_string) END),' ') AS value, 0 as identific, v.MA_Measure_Values_ID as valueid, (CASE v.VALUETYPE WHEN 'C' THEN 'checkbox' WHEN 'N' THEN 'number' ELSE 'text' END) as type, (CASE v.VALUETYPE WHEN 'N' THEN 'number' ELSE '' END) as validatetype, v_char as valuec" +
      ...
      "            union all" +
      "              select to_char('9999'::numeric), v.seqno, c.name || (CASE c.CRITICAL WHEN 'Y' THEN ' (PC)' ELSE '' END) AS name, ' ' as value, identific, '0' as valueid, 'hidden' as type,  'hidden' as validatetype, 'N' as valuec" +
      "              from ma_measure_time t, ma_measure_values v, ma_ccp c, " +
      ...
      "        order by seqno, name, measurehour::integer ";

    ResultSet result;
    Vector<EditCCPMeasureValuesValuesData> vector = new Vector<EditCCPMeasureValuesValuesData>(0);
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectEditCCPMeasureValuesValuesData.measurehour = UtilSql.getValue(result, "measurehour");
  ...
  public static EditCCPMeasureValuesValuesData[] selectValues(ConnectionProvider connectionProvider, String maMeasureGroupId)    throws ServletException {
    return selectValues(connectionProvider, maMeasureGroupId, 0, 0);
  }

  public static EditCCPMeasureValuesValuesData[] selectValues(ConnectionProvider connectionProvider, String maMeasureGroupId, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        select measurehour, name, max(value) as value, identific, seqno, valueid, type, valuec, validatetype" +
      "        from (" +
      ...
      "              and t.MA_MEASURE_GROUP_ID = ?" +
      "            union all" +
      "              select to_date('31/12/9999', 'DD/MM/YYYY'), v.seqno, c.name || (CASE c.CRITICAL WHEN 'Y' THEN ' (PC)' ELSE '' END) AS name, ' ' as value, identific, '0' as valueid, 'hidden' as type,  'hidden' as validatetype, 'N' as valuec" +
      "              from ma_measure_time t, ma_measure_values v, ma_ccp c, " +
      "                (select VALUE-1 as identific from ad_INTEGER " +
      "                 where VALUE<=(" +
      "                   select 10-count(*) from MA_Measure_time" +
