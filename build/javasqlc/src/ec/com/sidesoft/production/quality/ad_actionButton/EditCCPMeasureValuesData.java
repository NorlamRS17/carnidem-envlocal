...
static Logger log4j = Logger.getLogger(EditCCPMeasureValuesData.class);
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("ma_measure_shift_id") || fieldName.equals("maMeasureShiftId"))
  ...
  public static EditCCPMeasureValuesData[] select(ConnectionProvider connectionProvider, String maMeasureShiftID, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      "          SELECT " +
      "            s.MA_Measure_Shift_ID, " +
      "            g.MA_MEASURE_GROUP_ID AS groupid, " +
      "            cg.NAME AS groupname, " +
      "            '' AS name, " +
      "            '' AS Type, " +
      "            COALESCE(pp.name, ppmat.name) AS product " +
      "          FROM MA_Measure_Shift s " +
      "          JOIN MA_Measure_Group g " +
      "            ON g.MA_Measure_Shift_ID = s.MA_Measure_Shift_ID " +
      "          JOIN MA_CCP_Group cg " +
      "            ON g.MA_CCP_GROUP_ID = cg.MA_CCP_GROUP_ID " +
      "          LEFT JOIN m_product pp " +
      "            ON pp.m_product_id = s.EM_Spquly_Finishedprod_ID " +
      "          LEFT JOIN m_product ppmat " +
      "            ON ppmat.m_product_id = s.EM_Spquly_Product_ID " +
      "          WHERE s.MA_Measure_Shift_ID = ? " +
      "          ORDER BY g.MA_MEASURE_GROUP_ID, cg.seqno ";

    ResultSet result;
    Vector<EditCCPMeasureValuesData> vector = new Vector<EditCCPMeasureValuesData>(0);
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectEditCCPMeasureValuesData.maMeasureShiftId = UtilSql.getValue(result, "ma_measure_shift_id");
        ...
        objectEditCCPMeasureValuesData.product = UtilSql.getValue(result, "product");
