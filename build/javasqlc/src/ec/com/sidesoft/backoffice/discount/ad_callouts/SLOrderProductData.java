...
static Logger log4j = Logger.getLogger(SLOrderProductData.class);
  ...
  public static SLOrderProductData[] select(ConnectionProvider connectionProvider, String cOrderId, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "      SELECT C_ORDER_ID AS ID, DATEORDERED, C_BPARTNER_ID, M_PRICELIST_ID, '' AS DATEINVOICED, '' as C_CURRENCY_ID, '' as PRICELIMIT" +
      "          ,'' as PRICELIST, '' as PRICESTD" +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectSLOrderProductData.cCurrencyId = UtilSql.getValue(result, "c_currency_id");
        objectSLOrderProductData.pricelimit = UtilSql.getValue(result, "pricelimit");
        objectSLOrderProductData.pricelist = UtilSql.getValue(result, "pricelist");
        objectSLOrderProductData.pricestd = UtilSql.getValue(result, "pricestd");
  ...
  public static SLOrderProductData[] selectPrices(ConnectionProvider connectionProvider, String strMProductId, String strMPricelistId)    throws ServletException {
    return selectPrices(connectionProvider, strMProductId, strMPricelistId, 0, 0);
  }

  public static SLOrderProductData[] selectPrices(ConnectionProvider connectionProvider, String strMProductId, String strMPricelistId, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "      SELECT C_CURRENCY_ID, PRICELIMIT, PRICELIST, PRICESTD " +
      "  FROM M_PRODUCTPRICE PP " +
      "	  JOIN M_PRICELIST_VERSION PV ON PV.M_PRICELIST_VERSION_ID = PP.M_PRICELIST_VERSION_ID  " +
      "  	JOIN M_PRICELIST PL ON PL.M_PRICELIST_ID  = PV.M_PRICELIST_ID  " +
      "  WHERE PP.M_PRODUCT_ID = ? " +
      "  AND PL.M_PRICELIST_ID = ? " +
      "  AND PV.VALIDFROM = (SELECT MAX(SPV.VALIDFROM) FROM M_PRICELIST_VERSION SPV WHERE PL.M_PRICELIST_ID = SPV.M_PRICELIST_ID) ";
