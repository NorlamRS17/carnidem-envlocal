...
public class SFWPCUSPaidReceiptsFilter extends ProcessHQLQueryValidated {
  ...
  protected List<String> getQueryValidated(JSONObject jsonsent) throws JSONException {
    ...
    for (int i = 0; i < posts.length(); i++) {
      ...
      String columns = info.getString("columns").trim().replace("[", "").replace("]", "")
          .replaceAll("\"", "");
    ...
    if (isDomicilie) {
      ...
      orderTypeHql = "";
      domicilioQuery = " and (ord.sscmbSalesOrigin = 'WEB' or ord.sscmbSalesOrigin = 'CLC')"
          + " and ord.obposAppCashup is null" + " and ord.obposIslayaway = true"
    ...
    String hqlPaidReceipts = "select" + receiptsHQLProperties.getHqlSelect() + "from Order as ord "
        + "where $filtersCriteria and $hqlCriteria " + orderTypeHql
        + " and ord.client.id =  $clientId and ord.$orgId"
        + " and ord.obposIsDeleted = false and ord.documentStatus <> 'CJ' "
        + " and ord.documentStatus <> 'CA' and (ord.documentStatus <> 'CL' or ord.iscancelled = true)"//
        + domicilioQuery + " $orderByCriteria";
