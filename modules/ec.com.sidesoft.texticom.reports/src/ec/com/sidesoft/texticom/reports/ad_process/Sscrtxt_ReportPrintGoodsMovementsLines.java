...
public class Sscrtxt_ReportPrintGoodsMovementsLines extends HttpSecureAppServlet {
  private static Logger log4j = Logger.getLogger(Sscrtxt_ReportPrintGoodsMovementsLines.class);
  ...
  private void printPagePDF(HttpServletResponse response, VariablesSecureApp vars,
    ...
    String StrWindowdID = "170";
    String StrTableID = "324";
    String StrTransactionLineID = vars.getSessionValue("170|M_MOVEMENTLINE_ID");

    InternalMovementLine MMovementLineID = OBDal.getInstance().get(InternalMovementLine.class,
        StrTransactionLineID);
    ...
    // PrintWithh.add(
    // Restrictions.eq(SescrTemplateReport.PROPERTY_DOCTYPE, MMovementLineID.getSsrsCDoctype()));
    ...
    if (LstTemplate.get(0).getWindow().getId().equals(StrWindowdID)) {
      ...
      parameters.put("DOCUMENT_ID", MMovementLineID.getMovement().getId().toString());
