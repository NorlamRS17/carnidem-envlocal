...
public class SalesInoutAccountingTemplate extends DocInOutTemplate {
  ...
  static Logger log4jDocInOut = Logger.getLogger(SalesInoutAccountingTemplate.class);
  ...
  @Override
  public Fact createFact(DocInOut docInOut, AcctSchema as, ConnectionProvider conn, Connection con,
      VariablesSecureApp vars) throws ServletException {
    // create Fact Header
    Fact fact = new Fact(docInOut, as, Fact.POST_Actual);

    String Fact_Acct_Group_ID = SequenceIdData.getUUID();

    String strMInoutID = docInOut.Record_ID.toString();
    Record_ID = docInOut.Record_ID.toString();

    String stradOrgID = vars.getOrg();
    String stradClientID = vars.getClient();
    String stradUSerID = vars.getUser();

    ShipmentInOut ShpInoutLine = OBDal.getInstance().get(ShipmentInOut.class, strMInoutID);

    blnComWokrPr = ShpInoutLine.isSstpcIsconsworkp();

    DocumentType = ShpInoutLine.getDocumentType().getDocumentCategory();
    IsReturn = ShpInoutLine.getDocumentType().isReturn() ? "Y" : "N";

    p_lines = docInOut.loadLines(conn);
     ...
     * 
     * StrMInoutLine = docInOut.p_lines[i].m_TrxLine_ID;
     * 
     * fact.createLine(docInOut.p_lines[i], getAccount("2", StrMInoutLine, as, conn), "100", "", new
     * BigDecimal("200").multiply(new BigDecimal("2")).toString(), Fact_Acct_Group_ID,
     * docInOut.nextSeqNo(docInOut.getSeqNo()), docInOut.DocumentType, conn);
     * 
     * }
     */
    ...
    if (DocumentType.equals(AcctServer.DOCTYPE_MatShipment)) {
      ...
      for (int i = 0; p_lines != null && i < p_lines.length; i++) {
        StrMInoutLine = docInOut.p_lines[i].m_TrxLine_ID; // Revisa si el producto es almacenado
        ShipmentInOutLine ShpInoutLineSearch2 = OBDal.getInstance().get(ShipmentInOutLine.class,
            StrMInoutLine);
        if (ShpInoutLineSearch2.getProduct().isStocked()) { // Revisa si el producto es almacenado

          DocLine_Material line = (DocLine_Material) p_lines[i];
          Organization legalEntity = OBContext.getOBContext()
              .getOrganizationStructureProvider(stradClientID)
          ...
          if (!CostingStatus.getInstance().isMigrated()) {
            costCurrency = OBDal.getInstance().get(Client.class, stradClientID).getCurrency();
          ...
          if (cogsAccount == null) {
            StrMInoutLine = docInOut.p_lines[i].m_TrxLine_ID;
            ShipmentInOutLine ShpInoutLineSearch = OBDal.getInstance().get(ShipmentInOutLine.class,
                StrMInoutLine);

            // Contabiliza por el check Consumo Orden de Trabajo si esta marcado en la cabecera o en
            // las lineas.

            if (blnComWokrPr) {
              cogsAccount = getAccount("4", StrMInoutLine, as, conn);
            } else {
              if (ShpInoutLineSearch.isSstpcIsconsworkp()) {
                cogsAccount = getAccount("4", StrMInoutLine, as, conn);
              } else {
    ...
    // Purchasing
      ...
      for (int i = 0; p_lines != null && i < p_lines.length; i++) {
        ...
        Account notInvoicedReceiptsAccount = getAccount(AcctServer.ACCTTYPE_NotInvoicedReceipts, as,
            conn);
  ...
  public Account getAccount(String AcctType, String M_InoutLine_ID, AcctSchema as,
      ConnectionProvider conn) {
    if (Integer.parseInt(AcctType) < 1 || Integer.parseInt(AcctType) > 10)
      return null;

    ShipmentInOutLine ShpInoutLine = OBDal.getInstance().get(ShipmentInOutLine.class,
        M_InoutLine_ID);

    String validCombination_ID = "";

    if (blnComWokrPr) {
		ProductAccounts productAccount = ShpInoutLine.getProduct().getProductAccountsList().get(0);
		validCombination_ID = productAccount.getSstpcCostProcessProd() != null
				? productAccount.getSstpcCostProcessProd().getId()
				: productAccount.getProductCOGS().getId();
    } else {
