...
public class ProcessInvoice extends HttpSecureAppServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (vars.commandIn("DEFAULT")) {
			final String strWindowId = vars.getGlobalVariable("inpwindowId",
					"ProcessInvoice|Window_ID", IsIDFilter.instance);
			...
			final String strOrg = vars.getRequestGlobalVariable("inpadOrgId",
					"ProcessInvoice|Org_ID", IsIDFilter.instance);
			...
			if ((org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this,
					vars.getRole(), strTabId))
					|| !(Utility
							.isElementInList(Utility.getContext(this, vars, "#User_Client",
									strWindowId, accesslevel), strClient)
							&& Utility.isElementInList(Utility.getContext(this, vars, "#User_Org",
									strWindowId, accesslevel), strOrg))) {
			...
			final String strVoidInvoiceAcctDate = vars
					.getStringParameter("inpVoidedDocumentAcctDate");
			...
			try {
				...
				if ("CO".equals(strdocaction)) {
					...
					if (invoice.getBusinessPartner().getCurrency() == null) {
						String errorMSG = Utility.messageBD(this, "InitBPCurrencyLnk",
				...
				if (!strVoidInvoiceDate.isEmpty() && !strVoidInvoiceAcctDate.isEmpty()) {
					try {
						...
						log4j.error(
								"Not possible to parse the following date: " + strVoidInvoiceDate,
								pe);
						log4j.error("Not possible to parse the following date: "
								+ strVoidInvoiceAcctDate, pe);
					}
				...
				// In case of void a non paid invoice, create a dummy payment related to it with
				// zero amount
				...
				if ("RC".equals(strdocaction) && !invoice.isPaymentComplete()
					...
					try {
						...
						Ecscap_GeneralProcess generalProcess = null;
						if (isSOTrx) {
							OBCriteria<Ecscap_GeneralProcess> gPList = OBDal.getInstance()
									.createCriteria(Ecscap_GeneralProcess.class);
							gPList.add(
									Restrictions.eq(Ecscap_GeneralProcess.PROPERTY_ACTIVE, true));
							// gPList.addOrder(Order.desc(Ecscap_GeneralProcess.PROPERTY_UPDATED));

							if (gPList.list().size() > 0) {
								generalProcess = gPList.list().get(0);
							}

						}

						DocumentType docType = null;
						if (isSOTrx && generalProcess != null) {
							docType = generalProcess.getDocumentType();
						} else {
							docType = FIN_Utility.getDocumentType(invoice.getOrganization(),
						...
						if (isSOTrx && generalProcess != null) {
							bpFinAccount = generalProcess.getFinancialAccount();
						} else if (isSOTrx && invoice.getBusinessPartner().getAccount() != null
								&& FIN_Utility.getFinancialAccountPaymentMethod(
										invoice.getPaymentMethod().getId(),
							...
							bpFinAccount = invoice.getBusinessPartner().getAccount();
								...
								&& FIN_Utility.getFinancialAccountPaymentMethod(
										...
										invoice.getBusinessPartner().getPOFinancialAccount()
												.getId(),
										isSOTrx, invoice.getCurrency().getId(),
										invoice.getOrganization().getId()) != null) {
							...
							FinAccPaymentMethod fpm = FIN_Utility.getFinancialAccountPaymentMethod(
									invoice.getPaymentMethod().getId(), null, isSOTrx,
									invoice.getCurrency().getId(),
						...
						if (bpFinAccount == null) {
							...
							msg.setTitle(Utility.messageBD(this, "Error", vars.getLanguage()));
							msg.setMessage(
									OBMessageUtils.messageBD("APRM_NoFinancialAccountAvailable"));
							vars.setMessage(strTabId, msg);
							printPageClosePopUp(response, vars,
						...
						// Reversed invoice's date: voidDate in Purchase Invoice, new Date() in
						// Sales Invoice
						...
						if (!StringUtils.equals(invoice.getCurrency().getId(),
							...
							final ConversionRate conversionRate = FinancialUtils.getConversionRate(
									reversedDate, invoice.getCurrency(), bpFinAccount.getCurrency(),
						...
						if (isSOTrx && generalProcess != null) {
							paymentMethod = generalProcess.getPaymentMethod();
						}

						// Create dummy payment
						dummyPayment = dao.getNewPayment(isSOTrx, invoice.getOrganization(),
								docType, strPaymentDocumentNo, invoice.getBusinessPartner(),
								paymentMethod, bpFinAccount, "0", reversedDate,
								invoice.getDocumentNo(), invoice.getCurrency(), rate, null);
						...
						for (FIN_PaymentScheduleDetail psd : paymentScheduleDetails) {
							FIN_PaymentDetail pd = OBProvider.getInstance()
						...
						for (ConversionRateDoc conversionRateDoc : invoice
							...
							newConversionRateDoc
									.setOrganization(conversionRateDoc.getOrganization());
							...
							dummyPayment.getCurrencyConversionRateDocList()
									.add(newConversionRateDoc);
				...
				if ("RC".equals(strdocaction) && pinstance.getResult() != 0L) {
					try {
						...
						revInvoiceCriteria.add(
								Restrictions.eq(ReversedInvoice.PROPERTY_REVERSEDINVOICE, invoice));
						revInvoiceCriteria.setMaxResults(1);
						ReversedInvoice revInvoice = (ReversedInvoice) revInvoiceCriteria
								.uniqueResult();
						...
						if (voidingPrepaidInvoice) {
							...
							orderPaymentHQLQuery
									.append(" join fpd.fINPaymentScheduleDetailList fpsd");
							...
							dummyPayment = dao.getNewPayment(orderPayment.isReceipt(),
									invoice.getOrganization(), orderPayment.getDocumentType(),
									strPaymentDocumentNo, invoice.getBusinessPartner(),
									invoice.getPaymentMethod(), orderPayment.getAccount(), "0",
									voidDate != null ? voidDate : new Date(),
							...
							for (FIN_PaymentScheduleDetail fpsd : psdQuery.list()) {
								...
								FIN_PaymentSchedule orderPaymentSchedule = fpsd
								...
								if (invoiceFPDOrder != null) {
									...
									orderPSDHQLQuery
											.append(" where fpd.finPayment.id = :paymentId");
									orderPSDHQLQuery.append(" and fpsd.id <> :invoicePSDId");
									orderPSDHQLQuery
											.append(" and fpsd.invoicePaymentSchedule is null");
									OBQuery<FIN_PaymentScheduleDetail> orderPSDQuery = OBDal
											.getInstance()
											.createQuery(FIN_PaymentScheduleDetail.class,
									...
									FIN_PaymentScheduleDetail orderPSD = orderPSDQuery
											.uniqueResult();

									if (orderPSD == null) {
										// Order with no payment schedule detail, create a new one
										orderPSD = OBProvider.getInstance()
										...
										orderPSD.setAmount(
												orderPSD.getAmount().add(fpsd.getAmount()));
									}
									...
									invoice.setOutstandingAmount(
											invoice.getOutstandingAmount().add(fpsd.getAmount()));
								}
							}
							OBDal.getInstance().save(dummyPayment);
							...
							List<FIN_PaymentDetail> paymentDetails = new ArrayList<FIN_PaymentDetail>();
							List<FIN_PaymentScheduleDetail> paymentScheduleDetails = dao
									.getInvoicePendingScheduledPaymentDetails(
						...
						if (processPayment) {
							// Process dummy payment related with both actual invoice and reversed
							// invoice
				...
				if ("RE".equals(strdocaction) && pinstance.getResult() != 0L) {
					...
					if (invDesc != null) {
						final String creditMsg = Utility.messageBD(this,
				...
				if ("CO".equals(strdocaction)) {
					...
					try {
						...
						// payment method automation) or the business partner does not have a
						// default financial
						// account defined or invoice's payment method is not inside BP's financial
						// account or the business partner's currency is not equal to the invoice's
						// currency do
						// not cancel credit
						if (BigDecimal.ZERO.compareTo(invoice.getGrandTotalAmount()) != 0
								...
								&& (invoice.getBusinessPartner().getCurrency() != null
										&& StringUtils.equals(invoice.getCurrency().getId(), invoice
												.getBusinessPartner().getCurrency().getId()))) {
							creditPayments = dao.getCustomerPaymentsWithCredit(
									invoice.getOrganization(), invoice.getBusinessPartner(),
									invoice.isSalesTransaction(), invoice.getCurrency());
							if (creditPayments != null && !creditPayments.isEmpty()) {
								printPageCreditPaymentGrid(response, vars, strC_Invoice_ID,
										strdocaction, strTabId, strC_Invoice_ID, strdocaction,
		...
		} else if (vars.commandIn("USECREDITPAYMENTS")
				|| vars.commandIn("CANCEL_USECREDITPAYMENTS")) {
			final String strWindowId = vars.getGlobalVariable("inpwindowId",
					"ProcessInvoice|Window_ID", IsIDFilter.instance);
			final String strTabId = vars.getGlobalVariable("inpTabId", "ProcessInvoice|Tab_ID",
					IsIDFilter.instance);
			...
			if (vars.commandIn("CANCEL_USECREDITPAYMENTS")) {
				...
				strCreditPaymentIds = vars.getInParameter("inpCreditPaymentId",
						IsIDFilter.instance);
			}
			...
			if (strCreditPaymentIds != null && !strCreditPaymentIds.isEmpty()) {
				List<FIN_Payment> selectedCreditPayment = FIN_Utility
						.getOBObjectList(FIN_Payment.class, strCreditPaymentIds);
				HashMap<String, BigDecimal> selectedCreditPaymentAmounts = FIN_AddPayment
						.getSelectedBaseOBObjectAmount(vars, selectedCreditPayment,
								"inpPaymentAmount");
				try {
					OBContext.setAdminMode(true);
					...
					for (final FIN_Payment creditPayment : selectedCreditPayment) {
						...
						creditPayment
								.setUsedCredit(usedCreditAmt.add(creditPayment.getUsedCredit()));
						...
						description.append(
								String.format(Utility.messageBD(this, "APRM_CreditUsedinInvoice",
					...
					for (final FIN_PaymentScheduleDetail paymentScheduleDetail : dao
						...
						if (totalUsedCreditAmt.compareTo(allocatedAmt) > 0) {
							final BigDecimal pendingToAllocate = totalUsedCreditAmt
									.subtract(allocatedAmt);
					...
					final FIN_FinancialAccount bpFinAccount = isSalesTransaction
							...
							: invoice.getBusinessPartner().getPOFinancialAccount();
					// Calculate Conversion Rate
					final ConversionRate conversionRate = StringUtils.equals(
							invoice.getCurrency().getId(), bpFinAccount.getCurrency().getId())
									? null
									: FinancialUtils.getConversionRate(
					...
					final FIN_Payment newPayment = FIN_AddPayment.savePayment(null,
							isSalesTransaction, docType, strPaymentDocumentNo,
							invoice.getBusinessPartner(), invoice.getPaymentMethod(), bpFinAccount,
							"0", FIN_Utility.getDate(strPaymentDate), invoice.getOrganization(),
							invoice.getDocumentNo(), paymentScheduleDetails,
							paymentScheduleDetailsAmounts, false, false, invoice.getCurrency(),
							conversionRate != null ? conversionRate.getMultipleRateBy() : null,
							null);
					...
					for (final FIN_Payment creditPayment : selectedCreditPayment) {
						final BigDecimal usedCreditAmt = selectedCreditPaymentAmounts
								.get(creditPayment.getId());
						FIN_PaymentProcess.linkCreditPayment(newPayment, usedCreditAmt,
								creditPayment);
					}
	...
	void printPageDocAction(HttpServletResponse response, VariablesSecureApp vars,
		...
		if (strWindowId.equals(Purchase_Invoice_Window)) {
			// VOID action: Reverse sales/purchase invoice by default takes today as document date
			// and
			// accounting date.
			...
			Invoice invoice = (Invoice) OBDal.getInstance().getProxy(Invoice.ENTITY_NAME,
					strC_Invoice_ID);
			xmlDocument.setParameter("documentDate",
					OBDateUtils.formatDate(invoice.getInvoiceDate()));
		...
		FieldProvider[] dataDocAction = ActionButtonUtility.docAction(this, vars, strdocaction,
				"135", strdocstatus, strProcessing, stradTableId);
	...
	void printPageCreditPaymentGrid(HttpServletResponse response, VariablesSecureApp vars,
			String strC_Invoice_ID, String strdocaction, String strProcessing, String strdocstatus,
			String stradTableId, String strWindowId, String strTabId, Date invoiceDate,
		...
		PrintWriter out = response.getWriter();
		XmlDocument xmlDocument = xmlEngine
				.readXmlTemplate("org/openbravo/erpCommon/ad_actionButton/CreditPaymentGrid",
						discard)
				.createXmlDocument();
		xmlDocument.setParameter("css", vars.getTheme());
		xmlDocument.setParameter("language", "defaultLang=\"" + vars.getLanguage() + "\";");
	...
	private void printGrid(HttpServletResponse response, VariablesSecureApp vars, String invoiceId)
		...
		String[] discard = {};
		XmlDocument xmlDocument = xmlEngine
				.readXmlTemplate("org/openbravo/erpCommon/ad_actionButton/AddCreditPaymentGrid",
						discard)
				.createXmlDocument();
	...
	private FieldProvider[] getCreditPayments(Invoice invoice) {
		...
		try {
			...
			for (int i = 0; i < data.length; i++) {
				FieldProviderFactory.setField(data[i], "finCreditPaymentId",
						creditPayments.get(i).getId());
				FieldProviderFactory.setField(data[i], "documentNo",
						creditPayments.get(i).getDocumentNo());
				...
				FieldProviderFactory.setField(data[i], "paymentAmount",
						pendingToPay.compareTo(outStandingAmt) > 0 ? outStandingAmt.toString()
								: (pendingToPay.compareTo(BigDecimal.ZERO) > 0
										? pendingToPay.toString()
										: ""));
