...
public class FIN_PaymentProcess implements org.openbravo.scheduling.Process {
  ...
  private void updateCustomerCredit(BusinessPartner businessPartner, BigDecimal amount, boolean add) {
    ...
    finalConsumer.add(Restrictions.eq(Organization.PROPERTY_OBRETCOCBPARTNER , businessPartner ));   
  ...
  public String executeSql(String functionName, String paymentId) {
    String result = null;
    ...
    try {
      ...
      throw new OBException("FIN_PaymentProcess.executeSql: " + e.getMessage());
    } finally {
      try {
        conn.releasePreparedStatement(st);
        conn.destroy();
      } catch (Exception eF) {
        throw new OBException("FIN_PaymentProcess.executeSql: " + eF.getMessage());
      }
    }
    return result;
  }
