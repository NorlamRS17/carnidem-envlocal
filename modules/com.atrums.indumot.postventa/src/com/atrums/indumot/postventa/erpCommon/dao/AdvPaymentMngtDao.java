...
public class AdvPaymentMngtDao {
  ...
  public FIN_FinaccTransaction getFinancialTransaction(FIN_Payment payment) {
    ...
    if (transaction == null) {
      transaction = getNewFinancialTransaction(
          ...
          payment.getFinancialTransactionConvertRate(), payment.getAmount(), null);
    }
    return transaction;
  }
  ...
  public FIN_FinaccTransaction getNewFinancialTransaction(Organization organization,
      ...
      BigDecimal sourceAmount, String strNroDeposito) {
    ...
    if (strNroDeposito != null) {
      final OBQuery<FIN_Payment> obQuery = OBDal.getInstance().createQuery(FIN_Payment.class,
          "as p where p.id ='" + payment.getId() + "'");
      final List<FIN_Payment> fpaylist = obQuery.list();
      for (FIN_Payment fin_Payment : fpaylist) {
        fin_Payment.setAtecdpDeposito(strNroDeposito);
        // fin_Payment.setDpDeposito(strNroDeposito);
      }
    }
  ...
  public List<FIN_Payment> pagosCobrosPorDeposito(String numDeposito){
	  final StringBuilder whereClause = new StringBuilder();
	  final List<Object> parameters = new ArrayList<Object>();
	  whereClause.append(" as fp ");
	  whereClause.append(" where (fp.atecdpDeposito = ? ");
	  whereClause.append(" or fp.documentNo = ? )");
	  parameters.add(numDeposito);
	  parameters.add(numDeposito);
	  final OBQuery<FIN_Payment> obqP = OBDal.getInstance().createQuery(FIN_Payment.class,
	          whereClause.toString(), parameters);
	  return obqP.list();
