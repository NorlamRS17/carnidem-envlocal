...
public class PartialReturn extends DalBaseProcess {
	private static final Logger log4j = Logger.getLogger(PartialReturn.class);
	...
	public void doExecute(ProcessBundle bundle) throws Exception {
		...
		try {
			...
			final String recordId = (String) bundle.getParams().get("Atindpo_Postventalinea_ID");
			atindpo_postventalinea line = OBDal.getInstance().get(atindpo_postventalinea.class, recordId);

			final String quantity = (String) bundle.getParams().get("quantity");
			BigDecimal qty = new BigDecimal(0);
			if (quantity != null && !quantity.isEmpty()) {
				qty = new BigDecimal(quantity);
			}

			if (qty.compareTo(BigDecimal.ZERO) <= 0 || qty.compareTo(line.getSswosTransfer()) > 0) {
				throw new OBException(
						"La cantidad a devolver debe ser mayor a 0 y menor o igual a la cantidad transferida ("
								+ line.getSswosTransfer() + ")");
			}

			atindpo_postventa record = line.getAtindpoPostventa();
			...
			String result = Helper.executeSql("spsmvat_goodsmovement", recordId, "PR", qty);
