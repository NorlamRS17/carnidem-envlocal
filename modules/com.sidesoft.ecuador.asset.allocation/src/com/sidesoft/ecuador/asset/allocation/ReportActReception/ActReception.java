...
public class ActReception extends HttpSecureAppServlet {
  private static Logger log4j = Logger.getLogger(ActReception.class);
  ...
  private void printPagePDF(HttpServletResponse response, VariablesSecureApp vars,
    ...
    if (log4j.isDebugEnabled())
      log4j.debug("Output: Report_ActReception - pdf");

    ActReceptionData[] data = ActReceptionData.select(this, strDocumentId);// instancio un
                                                                               // objeto de la clase
                                                                               // data
    if (log4j.isDebugEnabled())
      log4j.debug("data: " + (data == null ? "null" : "not null"));
    if (data == null || data.length == 0)
      data = ActReceptionData.set();

    HashMap<String, Object> parameters = new HashMap<String, Object>();
    parameters.put("DOCUMENT_ID", strDocumentId);// nombre del parametro igual al del reporte

    String strReportName = "@basedesign@/com/sidesoft/ecuador/asset/allocation/ReportActReception/ActReception.jrxml";
