...
public class SprbiDaysEventListener extends EntityPersistenceEventObserver {
  private static Entity[] entities = {
      ModelProvider.getInstance().getEntity(SprbiDays.ENTITY_NAME) };
  ...
  public void onUpdate(@Observes EntityUpdateEvent event) {
    ...
    final SprbiDays record = (SprbiDays) event.getTargetInstance();
    String language = OBContext.getOBContext().getLanguage().getLanguage();
    ConnectionProvider conn = new DalConnectionProvider(false);

    String formattedDate = "";

    if (record != null && record.getShift().isSprbiConfigurationLines().equals(false)) {
      String message = Utility.messageBD(conn, "sprbi_config_head", language);
