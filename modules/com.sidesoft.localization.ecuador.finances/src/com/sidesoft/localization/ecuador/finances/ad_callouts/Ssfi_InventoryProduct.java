...
public class Ssfi_InventoryProduct extends SimpleCallout {
	...
	protected void execute(CalloutInfo info) throws ServletException {
		...
		if (strProductId != null && !strProductId.isEmpty()) {
			...
			OBCriteria<Costing> lastCost = OBDal.getInstance().createCriteria(Costing.class);
			lastCost.add(Restrictions.eq(Costing.PROPERTY_PRODUCT, product));
			lastCost.addOrder(Order.desc(Costing.PROPERTY_ENDINGDATE));
			lastCost.setMaxResults(1);

			if (lastCost.list().size() > 0) {
				Costing costing = lastCost.list().get(0);
				if (costing != null) {
					cost = costing.getCost();
				}
			}
		}
		info.addResult("inpcost", cost);
		...
		info.addResult("inpmAttributesetinstanceId_R", SLInOutLineProductData.attribute(this, strAttribute));
		...
		try {
			ComboTableData comboTableData = new ComboTableData(info.vars, this, "TABLE", "", "M_Product_UOM", "",
