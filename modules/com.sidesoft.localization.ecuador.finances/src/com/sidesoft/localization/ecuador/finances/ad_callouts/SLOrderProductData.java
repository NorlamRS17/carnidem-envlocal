...
class SLOrderProductData implements FieldProvider {
	...
	public static SLOrderProductData[] select(ConnectionProvider connectionProvider, String cOrderId)
			throws ServletException {
		return select(connectionProvider, cOrderId, 0, 0);
	}

	public static SLOrderProductData[] select(ConnectionProvider connectionProvider, String cOrderId, int firstRegister,
		...
		ResultSet result;
		Vector<SLOrderProductData> vector = new Vector<SLOrderProductData>(0);
		...
		try {
			...
			iParameter++;
			UtilSql.setValue(st, iParameter, 12, null, cOrderId);
	...
	public static SLOrderProductData[] selectInvoice(ConnectionProvider connectionProvider, String cInvoiceId)
			throws ServletException {
	...
	public static SLOrderProductData[] selectInvoice(ConnectionProvider connectionProvider, String cInvoiceId,
		...
		String strSql = "";
		strSql = strSql + "      SELECT C_INVOICE_ID AS ID, DATEINVOICED, C_BPARTNER_ID, M_PRICELIST_ID"
		...
		try {
			...
			QueryTimeOutUtil.getInstance().setQueryTimeOut(st, SessionInfo.getQueryProfile());
			iParameter++;
			UtilSql.setValue(st, iParameter, 12, null, cInvoiceId);
	...
	public static String hasSecondaryUOM(ConnectionProvider connectionProvider, String mProductId)
			throws ServletException {
		String strSql = "";
		strSql = strSql + "      SELECT (CASE COUNT(*) WHEN 0 THEN 0 ELSE 1 END) AS TOTAL "
				+ "      FROM M_PRODUCT_UOM " + "      WHERE M_PRODUCT_ID = ?" + "      AND ISACTIVE = 'Y'";
	...
	public static String getOrgLocationId(ConnectionProvider connectionProvider, String adUserClient, String adUserOrg)
			throws ServletException {
		String strSql = "";
		strSql = strSql + "	  	SELECT C_LOCATION_ID " + "	  		FROM AD_ORGINFO "
				+ "	  		WHERE AD_Client_ID IN (";
		strSql = strSql + ((adUserClient == null || adUserClient.equals("")) ? "" : adUserClient);
		strSql = strSql + ") " + "	  		AND AD_Org_ID IN (";
	...
	public static String getWarehouseOrg(ConnectionProvider connectionProvider, String cWarehouseID)
			throws ServletException {
		String strSql = "";
		strSql = strSql + "	  SELECT AD_ORG_ID" + "      FROM M_WAREHOUSE" + "      WHERE M_WAREHOUSE_ID = ?";
		...
		try {
			...
			QueryTimeOutUtil.getInstance().setQueryTimeOut(st, SessionInfo.getQueryProfile());
			iParameter++;
			UtilSql.setValue(st, iParameter, 12, null, cWarehouseID);
	...
	public static String getWarehouseOfOrg(ConnectionProvider connectionProvider, String adClientId, String adOrgId)
			throws ServletException {
		String strSql = "";
		strSql = strSql + "        select min(w.m_warehouse_id)" + "        from m_warehouse w"
				+ "        where w.ad_client_id=?" + "          and (ad_isorgincluded(?,w.ad_org_id,?)<>-1"
