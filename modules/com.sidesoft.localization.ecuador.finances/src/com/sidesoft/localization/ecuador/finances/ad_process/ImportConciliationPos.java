...
public class ImportConciliationPos extends IdlServiceJava {
  ...
  public BaseOBObject createConciliationPos (final String identifier, final String financialaccountpay,
    ...
    String line = "10";
	//FIN_Payment payment = OBDal.getInstance().get(FIN_Payment.class, finacctransaction.getId());
    ...
    try {
      ...
      finacctransaction.setDescription(identifier);  
      finacctransaction.setCostCenter(costcenter);
      finacctransaction.setSscccinReference(description);
      ...
      finacctransaction.setLineNo(Long.valueOf(line));
     
      OBDal.getInstance().save(finacctransaction);
      OBDal.getInstance().flush();
    } catch (Exception e) {
      throw new OBException("Ha sucedido un error al guardar el registro "+e.getMessage());
      //e.printStackTrace();
    }
    ...
    if (!financialaccountrec.equals("") ) {
      ...
      //FIN_Payment paymentrec = OBDal.getInstance().get(FIN_Payment.class, finacctransactionrec.getId());
      ...
      try {
        ...
        finacctransactionrec.setDescription(identifier); 
        finacctransactionrec.setSscccinReference(description);
        ...
        finacctransactionrec.setLineNo(Long.valueOf(line));
