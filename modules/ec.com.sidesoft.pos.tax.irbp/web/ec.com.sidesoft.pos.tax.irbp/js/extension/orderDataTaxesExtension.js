...
(function () {
  ...
  var getTaxRateNumber = function (taxRate, isIrbp) {

      var rate = new BigDecimal(String(taxRate)); // 10
      if(isIrbp){
         return new BigDecimal(String(taxRate));
      }
      ...
      return Promise.all(_.map(receipt.get('lines').models, function (line) {
        ...
        _.forEach(line.get('taxes'), function (tax) {
          ...
          line.set({
            ...
            'linerate': OB.DEC.toNumber(linerate.add(getTaxRateNumber(tax.taxRate, false)))
          }, {
            silent: true
          });
        });
      ...
      return findTaxesCollection(receipt, line, taxCategory).then(function (coll) {
        ...
        var callbackTaxRate = function (taxRate, taxIndex, taxList) {
            if (!taxRate.get('summaryLevel')) {
              ...
              var rate = getTaxRateNumber(taxRate.get('rate'), false);
  ...
  var calcProductTaxesExcPrice = function (receipt, line, taxCategory, linepricenet, linenet, discountedprice, discountedNet) {
      ...
      return findTaxesCollection(receipt, line, taxCategory).then(function (coll) {
        ...
        var pricenetirbp = new BigDecimal(String('1'));
        var plasticUnitsBom = OB.DEC.mul((line.get('product').get('sirbpPlasticunitsbom')||0), line.get('qty'));
        var packageType = line.get('product').get('slplagPackage')||'B';        
        const irbpLabel = 'IRBP';
        if(plasticUnitsBom > 0){
           packageType = 'P';
        }
        // second calculate tax lines.
        var taxes = receipt.get('taxes');
        ...
        var callbackNotInclTax = function (taxRate, taxIndex, taxList) {
            ...
            //implemented irbp tax             
            if (!taxRate.get('summaryLevel')) {

              var taxId = taxRate.get('id');
              var isIrbp = false;
              if((taxRate.get('name').toUpperCase()).includes(irbpLabel)){
                 if(packageType === 'P' || packageType === 'SFP'){
                    isIrbp = true;
                 }
              }                 
              var rate = getTaxRateNumber(taxRate.get('rate'), isIrbp);
          //var net = OB.DEC.mul(pricenetAux, line.get('qty')); //=== discountedNet
          console.log('****************************');
          console.log(taxRate);
              console.log('1.1---> packageType:: '+ packageType);
              if((taxRate.get('name').toUpperCase()).includes(irbpLabel)){
                 if(packageType === 'P' || packageType === 'SFP'){
            console.log('1.1.0---> packageType:: '+ packageType+'   taxName :: '+taxRate.get('name'));
            var net = OB.DEC.mul(pricenetirbp, (plasticUnitsBom > 0 ? plasticUnitsBom : line.get('qty'))); //=== discountedNet
             } else {
               var net = OB.DEC.mul(new BigDecimal(String('0')), line.get('qty')); //=== discountedNet
             }      
          } else{
             console.log('1.1.1---> packageType:: '+ packageType+'   taxName :: '+taxRate.get('name'));
         var net = OB.DEC.mul(pricenetAux, line.get('qty')); //=== discountedNet
          }
              ...
              if (taxRate.get('cascade')) {
                ...
                discountedGross = discountedGross.add(new BigDecimal(OB.DEC.toNumber(new BigDecimal(String(net)).multiply(rate)).toString()));
                console.log('************************************* linegross');
                console.log(linegross);
                console.log(discountedGross);
              }
              ...
              console.log('1...net:: '+ net);
              console.log('1...amount:: '+ amount);              
              if (taxes[taxId]) {
              console.log('1.1.0..taxes[taxId].net:: '+ taxes[taxId].net);
              console.log('1.2---> packageType:: '+ packageType);
                if((taxes[taxId].name.toUpperCase()).includes(irbpLabel)){
                  if(packageType === 'P' || packageType === 'SFP'){
                   console.log('1.2.0---> packageType:: '+ packageType+'   taxName :: '+taxes[taxId].name);
                   taxes[taxId].net = OB.DEC.add(taxes[taxId].net, OB.DEC.mul(pricenetirbp, (plasticUnitsBom > 0 ? plasticUnitsBom : line.get('qty'))));
                  } else {
                    taxes[taxId].net = OB.DEC.add(taxes[taxId].net, OB.DEC.mul(new BigDecimal(String('0')), line.get('qty')));
                  } 
                } else {
                   console.log('1.2.1---> packageType:: '+ packageType+'   taxName :: '+taxes[taxId].name);
              ...
              console.log('1.1..name:: '+ name);
              console.log('1.1..rate:: '+ rate);              
              console.log('1.1..net:: '+ net);
              console.log('1.1..amount:: '+ amount);
              console.log('1.1..taxes[taxId].amount:: '+ taxes[taxId].amount);
              console.log('1.1..taxes[taxId].net:: '+ taxes[taxId].net);      
              console.log('1.1..discountedprice*qty:: '+OB.DEC.mul(discountedprice, line.get('qty')))                                      
              } else {
                taxes[taxId] = {};
                taxes[taxId].name = taxRate.get('name');
                taxes[taxId].rate = taxRate.get('rate');
                taxes[taxId].docTaxAmount = taxRate.get('docTaxAmount');
                taxes[taxId].net = net;
        console.log('2...name:: '+ taxRate.get('name'));
        console.log('2...rate:: '+ taxRate.get('rate'));              
        console.log('2...net:: '+ net);
        ...
        console.log('line gross :: '+line.get('gross')+', linegross :: '+OB.DEC.toNumber(linegross)+', linenet :: '+linenet);
        console.log('gross :: '+OB.DEC.add(line.get('gross'), OB.DEC.sub(OB.DEC.toNumber(linegross), linenet)));        
        console.log('line discountedGross :: '+line.get('discountedGross')+', discountedGross :: '+OB.DEC.toNumber(discountedGross)+', discountedNet :: '+discountedNet);
        console.log('discountedGross :: '+OB.DEC.add(line.get('discountedGross'), OB.DEC.sub(OB.DEC.toNumber(discountedGross), discountedNet)));                
        line.set({
      ...
      return Promise.all(_.map(frozenLines.models, function (line) {
        ...
        _.forEach(receipt.get('taxes'), function (tax, taxid) {
          if (tax.docTaxAmount === 'D') {
              var isIrbp = false;
              if((tax.name.toUpperCase()).includes(irbpLabel)){
                    isIrbp = true;
              }            
            // Adjust taxes in case of taxes at doc level...
            newAmount = OB.DEC.mul(tax.net, getTaxRateNumber(tax.rate, isIrbp));
