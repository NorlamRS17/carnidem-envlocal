  ...
  private void buildData(HttpServletResponse response, VariablesSecureApp vars, String strDate,
    ...
    try {
      ...
      if (strCostType != null && !isWarehouseConsolidation) {
        data = vars.commandIn("PDF") ? ReportValuationStockData.select(readOnlyCP,
            vars.getLanguage(), strCurrencyId, strLegalEntity, strDateNext, strMaxAggDate,
            processTime, dateFormat, orgIds, strWarehouseIncluded, strCostOrg, strCostClientId,
            strCostType, warehouseDimension, strCategoryProduct) : ReportValuationStockData
            .selectXLS(readOnlyCP, vars.getLanguage(), strCurrencyId, strLegalEntity, strDateNext,
                strMaxAggDate, processTime, dateFormat, orgIds, strWarehouseIncluded, strCostOrg,
                strCostClientId, strCostType, warehouseDimension, strCategoryProduct);
      } else if (strCostType == null && !isWarehouseConsolidation) {
        data = vars.commandIn("PDF") ? ReportValuationStockData.selectWithoutCost(readOnlyCP,
            vars.getLanguage(), strCurrencyId, strLegalEntity, strDateNext, strMaxAggDate,
            processTime, dateFormat, orgIds, strWarehouseIncluded, strCategoryProduct)
            : ReportValuationStockData.selectWithoutCostXSL(readOnlyCP, vars.getLanguage(),
        ...
        data = vars.commandIn("PDF") ? ReportValuationStockData.selectClusteredByWarehouse(
            readOnlyCP, vars.getLanguage(), strDateNext, strLegalEntity, strCurrencyId,
            strMaxAggDate, processTime, dateFormat, orgIds, strWarehouseIncluded,
            strCategoryProduct, strCostOrg, strCostClientId, strCostType, warehouseDimension,
            filterOrg.getId()) : ReportValuationStockData.selectClusteredByWarehouseXLS(readOnlyCP,
            vars.getLanguage(), strDateNext, strLegalEntity, strCurrencyId, strMaxAggDate,
            processTime, dateFormat, orgIds, strWarehouseIncluded, strCategoryProduct, strCostOrg,
            strCostClientId, strCostType, warehouseDimension, filterOrg.getId());
      } else {
        data = vars.commandIn("PDF") ? ReportValuationStockData
            .selectClusteredByWarehouseWithoutCost(readOnlyCP, vars.getLanguage(), strCurrencyId,
                strLegalEntity, strDateNext, strMaxAggDate, processTime, dateFormat, orgIds,
                strWarehouseIncluded, strCategoryProduct) : ReportValuationStockData
            .selectClusteredByWarehouseWithoutCostXLS(readOnlyCP, vars.getLanguage(),
                strCurrencyId, strLegalEntity, strDateNext, strMaxAggDate, processTime, dateFormat,
                orgIds, strWarehouseIncluded, strCategoryProduct);
      }
