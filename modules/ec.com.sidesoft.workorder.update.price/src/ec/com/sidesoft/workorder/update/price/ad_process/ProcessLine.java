...
public class ProcessLine extends DalBaseProcess {
  OBError message;
  ...
  // SSWOUP_PR
  // SSWOUP_AP
  // private static Logger log4j1 = Logger.getLogger(ProcessLine.class);
  ...
  public void processRequest(ProcessBundle bundle) throws Exception {
    ...
    if (status != false) {
      boolean status_p = processLines(record);
      messageT = "El proceso se realizo de manera exitosa.";
      record.setAlertStatus("SSWOUP_PR");
  ...
  public boolean processLines(UpdatePriceWO record) throws OBException, ServletException {
    boolean status = true;
    ...
    for (UpdatePriceLineWO line : obc.list()) {
      UpdatePriceWoData[] data = UpdatePriceWoData.linesWorkOrder(this.conn,
          record.getSSWOUP().getId(), line.getWorkorderNo());
      for (UpdatePriceWoData line_wo : data) {

        String pTarifa = line_wo.preciotarifa;
        String pUnitario = line_wo.preciounitario;

        if (!pTarifa.equals("0") && !pTarifa.equals("0")) {
          UpdatePriceWoData.updateLinesWorkOrder(this.conn, line_wo.preciounitario,
              line_wo.preciotarifa, line_wo.preciounitario, line_wo.linea);
        }
      }
      UpdateWoData[] ordenwo = UpdateWoData.totalWorkOrder(this.conn, record.getSSWOUP().getId(),
          line.getWorkorderNo());
      for (UpdateWoData wo : ordenwo) {
        UpdateWoData.updateTotalWorkOrder(this.conn, wo.total, wo.iva, wo.orden);
      }
    }
    return status;
  }
