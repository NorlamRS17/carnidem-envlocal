...
public class SSWOS_Movement_Product extends SimpleCallout {
  ...
  protected void execute(CalloutInfo info) throws ServletException {
    ...
    info.addResult("inpmLocatorId_R",
        SSWOSInOutLineProductData.locator(this, strLocator, info.vars.getLanguage()));
    ...
    info.addResult("inpmAttributesetinstanceId_R",
        SSWOSInOutLineProductData.attribute(this, strAttribute));
    ...
    try {
      ...
      if (product != null) {
        ...
        info.addResult("inpemSswosIdentifier", product.getSearchKey());
    ...
    String strHasSecondaryUOM = SSWOSOrderProductData.hasSecondaryUOM(this, strMProductID);
    ...
    String strMPricelistId = info.vars.getStringParameter("inpmPricelistId");
    String strPostVentaID = info.vars.getStringParameter("inpatindpoPostventaId");
    atindpo_postventa objPostVenta = OBDal.getInstance().get(atindpo_postventa.class,
        strPostVentaID);
    SSWOSProductIDData[] data = SSWOSProductIDData.select(this, strMProductID,
        objPostVenta.getPriceList().getId());
    if (data.length != 0) {
      if (data[0].pricelist != null && data[0].pricestd != null) {
        info.addResult("inpatindpoUnitprice", data[0].pricelist);
        info.addResult("inpatindpoPricelist", data[0].pricestd);
        info.addResult("inpcTaxId", data[0].cTaxId);
      } else {
