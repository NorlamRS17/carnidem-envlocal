...
public class GeneratePurchaseSettlement extends DalBaseProcess {
					...
					if (strXML.contains("<?xml version")) {
						if (strDocType.equals("03")) {
							...
							if (strWSResult != null) {
								...
								String strClaveAcceso = null, strUrlXML = null, strUrlRide = null, strStatusFe = "GE", strDateAut=null;
								...
								if (strWSEstado.trim().toUpperCase()
									...
									try {
										...
										if(strResultado.length > 6 ) {
											strStatusFe = strResultado[6].split(":", 2)[1];
											strDateAut =  strResultado.length > 7 ? strResultado[7].split(":", 2)[1] : null;
											log4j.debug("strStatusFe " + strStatusFe);
											log4j.debug("strDateAut " + strDateAut);
										}
										...
										try {
											...
											UpdateInvoice(conn, strInvoiceID,
													...
													strUrlRide,strStatusFe, strDateAut,
										...
										if(strResultado.length > 8 ) {
											strResult = strResultado[8].split(":", 2)[1];
										}else {
											strResult="GENERADO";
										}
										strTipoResult = "S";
	...
	public static void UpdateInvoice(ConnectionProvider connectionProvider,
			...
			String strURLRide, String strStatusFe, String strDateAut, boolean boolKeyAccessGenerate) {
		String strSql = null;
		if(strStatusFe.equals("NO AUTORIZADO") || strStatusFe.equals("DEVUELTA")) {
			strStatusFe = "NA";
		}
		Boolean applyDateAu = false;
		String sqlDate = "";
		if (strDateAut != null 
		        && !"null".equalsIgnoreCase(strDateAut.trim()) 
		        && !strDateAut.trim().isEmpty()) {
			applyDateAu = true;
			sqlDate = ", EM_Eei_Fechaautotext = ? , EM_Eei_Fechaauto = ? ";
		}

		if (!boolKeyAccessGenerate) {
			strSql = "update c_invoice set em_eei_codigo_2=?,em_eei_numauto_2=?, em_eei_urlxml_2=?, em_eei_urlride_2=?, em_eei_status_2=? "+sqlDate+"  where c_invoice_id =?";
		} else {
			strSql = "update c_invoice set em_eei_numauto_2=?, em_eei_urlxml_2=?, em_eei_urlride_2=?, em_eei_status_2=? "+sqlDate+" where c_invoice_id =?";
		}

		int updateCount = 0;
		PreparedStatement st = null;
		Timestamp tsDateAut = null;
		
		if (applyDateAu) {
			tsDateAut = Timestamp.valueOf(strDateAut);
		}
		try {
			st = connectionProvider.getPreparedStatement(strSql);
			if (!boolKeyAccessGenerate) {
				st.setString(1, strClaveAcceso);
				st.setString(2, strClaveAcceso);
				st.setString(3, strURLXml);
				st.setString(4, strURLRide);
				st.setString(5, strStatusFe);
				if (applyDateAu) {
					st.setTimestamp(6, tsDateAut);
					st.setTimestamp(7, tsDateAut);
					st.setString(8, strInvID);
				}else {
					st.setString(6, strInvID);	
				}
				...
				st.setString(4, strStatusFe);
				if (applyDateAu) {
					st.setTimestamp(5, tsDateAut);
					st.setTimestamp(6, tsDateAut);
					st.setString(7, strInvID);
				}else {
					st.setString(5, strInvID);
				}			
			}
