...
public class GenerateFE extends DalBaseProcess {
	...
	public static void UpdateInvoice(ConnectionProvider connectionProvider,
		...
		if (!boolKeyAccessGenerate) {
			strSql = "update c_invoice set em_eei_codigo=?, em_eei_numauto=?, em_eei_urlxml=?,em_eei_urlride=?, em_eei_status=? "+ sqlDate +"  where c_invoice_id =?";
		} else {
			strSql = "update c_invoice set em_eei_numauto=?, em_eei_urlxml=?, em_eei_urlride=?, em_eei_status=? "+sqlDate +" where c_invoice_id =?";
		}
