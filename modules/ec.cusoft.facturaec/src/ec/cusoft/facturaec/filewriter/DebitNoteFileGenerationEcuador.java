...
public class DebitNoteFileGenerationEcuador extends AbstractFileGeneration implements OBEInvoice_I,
  ...
  public String generateFile(Invoice invoice, String rootDirectory, String lang) throws Exception {
    ...
    {
    	strCodDocModificado = invoice.getDocumentType().getDocumentCategory() == "AR factura" ? "01"
    ...
    String strNumDocModificado;
    
    if (!docInvoice.isEeiNoReferenceInvoice())
    {
    	strNumDocModificado = invoice.getEeiRefInv().getDocumentNo();// .substring(8);
    	System.out.println("****************** La orden de referencia de la factura referenciada es : "
    	        + invoice.getEeiRefInv().getOrderReference() + "**************");
    }
    else
    {
    	strNumDocModificado = invoice.getEeiInvoiceNum().substring(8);
    }
    ...
    {
    	strNumDocModificado = invoice.getEeiInvoiceNum();
    }
