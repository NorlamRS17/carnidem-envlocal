...
public class Rpt_EntryCellar extends HttpSecureAppServlet {
  private static Logger log4j = Logger.getLogger(Rpt_EntryCellar.class);
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
    ...
    if (vars.commandIn("DEFAULT")) {
      String strDocumentId = vars.getSessionValue("184|M_INOUT_ID");
  ...
  private void printPagePDF(HttpServletResponse response, VariablesSecureApp vars,
    ...
    if (log4j.isDebugEnabled())
      log4j.debug("Output: Rpt_EntryCellar - pdf");
    
    RptEntryCellarData[] data = RptEntryCellarData.select(this, strDocumentId);
    if (log4j.isDebugEnabled())
      log4j.debug("data: " + (data == null ? "null" : "not null"));
    if (data == null || data.length == 0)
      data = RptEntryCellarData.set();

    HashMap<String, Object> parameters = new HashMap<String, Object>();
    parameters.put("DOCUMENT_ID", strDocumentId);

    String strReportName = "@basedesign@/ec/com/sidesoft/special/customization/ecuapack/ad_reports/Rpt_EntryCellar.jrxml";
