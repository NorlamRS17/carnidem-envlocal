...
public class ImportPhysicalInventory extends IdlServiceJava {
  ...
  public BaseOBObject createphysicalinventory(final String documentno, final String line,
    ...
    InventoryCount inventoryid = findDALInstance(false, InventoryCount.class,
        new Value(InventoryCount.PROPERTY_SSINDOCUMENTNO, documentno));
    ...
    if (cost == null || cost.equals("")) {
        throw new OBException("Costo no existe");
    } else {
    	try {
    		 newCost = new BigDecimal(cost);
    	}catch (Exception e) {
    		throw new OBException("Costo: "+cost+" no válido");
		}
    }
    ...
    try {
      ...
      inventoryline.setLineNo(Long.valueOf(line));
      ...
      inventoryline.setCost(newCost);

      OBDal.getInstance().save(inventoryline);
      OBDal.getInstance().flush();
      OBDal.getInstance().refresh(inventoryline);
    } catch (Exception e) {
