...
public class SL_Invoice_Amt extends SimpleCallout {
  ...
  protected void execute(CalloutInfo info) throws ServletException {
    ...
    BigDecimal newDiscount = info.getBigDecimalParameter("inpemSsfiDiscount");
	...
	if (StringUtils.equals(strChanged, "inppriceactual") || StringUtils.equals(strChanged, "inplinenetamt")) {
 ...
 // Calculate Discount
 	if (StringUtils.equals(strChanged, "inppricelist") || StringUtils.equals(strChanged, "inppriceactual")
 		...
 		BigDecimal netPriceList = isTaxIncludedPriceList ? grossPriceList : priceList;
 		BigDecimal unitPrice = isTaxIncludedPriceList ? baseGrossUnitPrice : priceStd;
 		BigDecimal discount = netPriceList.compareTo(BigDecimal.ZERO) == 0 || !calcDiscount ? BigDecimal.ZERO
 				: netPriceList.subtract(unitPrice).multiply(new BigDecimal("100")).divide(netPriceList, stdPrecision,
 						RoundingMode.HALF_EVEN);
 		log4j.debug("Discount rounded: " + discount.toString());
 		info.addResult("inpemSsfiDiscount", discount);

 	} else if (StringUtils.equals(strChanged, "inpemSsfiDiscount")) {

 		// Calculate PriceStd, PriceActual, GrossPriceStd, GrossUnitPrice
 		BigDecimal origDiscount = BigDecimal.ZERO;
 		BigDecimal netPriceList = isTaxIncludedPriceList ? grossPriceList : priceList;
 		if (netPriceList.compareTo(BigDecimal.ZERO) != 0) {
 			BigDecimal baseUnitPrice = isTaxIncludedPriceList ? baseGrossUnitPrice : priceStd;
 			origDiscount = priceList.subtract(baseUnitPrice).multiply(new BigDecimal("100")).divide(netPriceList,
 					stdPrecision, RoundingMode.HALF_UP);
 		}

 		if (origDiscount.compareTo(newDiscount) != 0) {
 			BigDecimal baseUnitPrice = netPriceList
 					.subtract(netPriceList.multiply(newDiscount).divide(new BigDecimal("100")))
 					.setScale(pricePrecision, RoundingMode.HALF_UP);
 			if (isTaxIncludedPriceList) {
 				grossUnitPrice = PriceAdjustment.calculatePriceActual(invoice, product, qtyInvoice, baseUnitPrice);
 				...
 				BigDecimal netAmount = FinancialUtils.calculateNetAmtFromGross(strTaxId, grossAmount, stdPrecision,
 						taxBaseAmt);
    ...
    if (!StringUtils.equals(strChanged, "inplinenetamt")
      ...
      info.addResult("inplinenetamt", lineNetAmt);
		...
		if(PriceAdjustment.discountPrice(invoice, product, qtyInvoice, priceStd)!=new BigDecimal ("0")){
		 discountprice = PriceAdjustment.discountPrice(invoice, product, qtyInvoice, priceStd);
		}

		//% Discount 
	  info.addResult("inpemSsbodDiscountRate", discountprice);
