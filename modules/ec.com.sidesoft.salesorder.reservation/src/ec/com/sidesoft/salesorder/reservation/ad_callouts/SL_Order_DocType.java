		...
		if (StringUtils.isNotEmpty(strDocTypeTarget)) {
			...
			if (dataNew != null && dataNew.length > 0) {
				...
				if (!StringUtils.equals(docSubTypeSO, "OB") && StringUtils.equals(strOldDocSubTypeSO, "OB")) {
					...
					info.addResult("inpdescription",
							FormatUtilities.replaceJS(Utility.messageBD(this, "Quotation", info.vars.getLanguage())
				...
				paymentRule = "P";
				invoiceRule = StringUtils.equals(docSubTypeSO, "PR") || StringUtils.equals(docSubTypeSO, "WI") ? "I"
						: "D";
				deliveryRule = "A";
			...
			if (isSOTrx) {
				...
				if (docType.getSsorresInvoiceRule() != null) {
					invoiceRule = docType.getSsorresInvoiceRule();
				}
			}
			...
			if (!StringUtils.equalsIgnoreCase(docSubTypeSO, "WR")) {
				...
				if (dataBP != null && dataBP.length > 0) {
					...
					if(docType == null || docType.getSsorresInvoiceRule() == null) {
						invoiceRule = StringUtils.equals(docSubTypeSO, "PR") || StringUtils.equals(docSubTypeSO, "WI") ? "I"
				...
				if (l != null && l.length > 0) {
					...
					for (int i = 0; i < l.length; i++) {
						info.addSelectResult(l[i].getField("id"), FormatUtilities.replaceJS(l[i].getField("name")),
