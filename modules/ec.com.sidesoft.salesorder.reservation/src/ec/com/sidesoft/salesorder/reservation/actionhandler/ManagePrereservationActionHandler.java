 ...
 ************************************************************************
 */
package ec.com.sidesoft.salesorder.reservation.actionhandler;
...
public class ManagePrereservationActionHandler extends BaseProcessActionHandler {
	...
	private void managePrereservedStockLines(JSONObject jsonRequest, OrderLine pol, List<String> idList)
			throws JSONException {
		JSONArray selectedLines = jsonRequest.getJSONObject("_params").getJSONObject("grid").getJSONArray("_selection");
		...
		for (int i = 0; i < selectedLines.length(); i++) {
			...
			String strReservationStockId = selectedLine.get(PrereservationManualPickEdit.PROPERTY_RESERVATIONSTOCK)
					.equals(null) ? "" : selectedLine.getString(PrereservationManualPickEdit.PROPERTY_RESERVATIONSTOCK);
			boolean existsReservationStock = StringUtils.isNotBlank(strReservationStockId);

			final String strSOLId = selectedLine.getString(PrereservationManualPickEdit.PROPERTY_SALESORDERLINE);
			...
			final boolean allocated = selectedLine.getBoolean(PrereservationManualPickEdit.PROPERTY_ALLOCATED);
