...
public class SSFC_Order_BPartner extends SimpleCallout {
 ...
 // column salesRep
    ...
    OBCriteria<BusinessPartner> obcSalesRep = OBDal.getInstance().createCriteria(BusinessPartner.class);
    obcSalesRep.add(Restrictions.eq(BusinessPartner.PROPERTY_ID, strBPartner));
    obcSalesRep.setMaxResults(1);
    BusinessPartner salesPartner = (BusinessPartner) obcSalesRep.uniqueResult();
    
    info.addResult("inpsalesrepId", salesPartner.getSalesRepresentative());
    //fin salesRep
    
    DocumentType docType = null;
    if (strIsSOTrx.equals("Y")) {
      docType = OBDal.getInstance().get(DocumentType.class, strDocTypeTarget);
      if (docType.getSsorresInvoiceRule() != null) {
        strInvoiceRule = docType.getSsorresInvoiceRule();
      }
    }
    ...
    if (data != null && data.length > 0) {
      strDeliveryRule = data[0].deliveryrule.equals("")
          ...
          : data[0].deliveryrule;
      strUserRep = SSFCOrderBPartnerData.userIdSalesRep(this, data[0].salesrepId);
      if (StringUtils.isEmpty(strUserRep)) {
        strUserRep = info.vars.getUser();
      }
      SSFCOrderDocTypeData[] docTypeData = SSFCOrderDocTypeData.select(this, strDocTypeTarget);
      ...
      if (strInvoiceRule == null || strInvoiceRule.isEmpty()) {
      ...
      if (strPriceList.equalsIgnoreCase("")) {
        strPriceList = SSFCOrderBPartnerData.defaultPriceList(this, strIsSOTrx,
            info.vars.getClient());
      }
    ...
    if (td != null && td.length > 0) {
      ...
      String strMwarehouse = strIsSOTrx.equals("N")
          ? SSFCOrderBPartnerData.mWarehouse(this, strBPartner)
          : SSFCOrderBPartnerData.mWarehouSSFCOnhand(this, strOrgId);
      ...
      for (int i = 0; i < td.length; i++) {
        // If user's default warehouse [Login Warehouse] is present in the warehouse
        // list, set it as
        // selected
    ...
    /*
     * FieldProvider[] tld = null; try { ComboTableData comboTableData = new
     * ComboTableData(info.vars, this, "TABLE", "", "AD_User SalesRep", "", Utility.getContext(this,
     * info.vars, "#AccessibleOrgTree", "SEOrderBPartner"), Utility.getContext(this, info.vars,
     * "#User_Client", "SEOrderBPartner"), 0); Utility.fillSQLParameters(this, info.vars, null,
     * comboTableData, "SEOrderBPartner", ""); tld = comboTableData.select(false); comboTableData =
     * null; } catch (Exception ex) { throw new ServletException(ex); }
     */
    OBCriteria<BusinessPartner> obcBp = OBDal.getInstance().createCriteria(BusinessPartner.class);
    obcBp.add(Restrictions.eq(BusinessPartner.PROPERTY_ID, strBPartner));
    obcBp.setMaxResults(1);
    BusinessPartner partner = (BusinessPartner) obcBp.uniqueResult();
    if (partner != null && partner.getSalesRepresentative() != null) {
      OBCriteria<User> obcUS = OBDal.getInstance().createCriteria(User.class);
      obcUS.add(Restrictions.eq(User.PROPERTY_BUSINESSPARTNER, partner.getSalesRepresentative()));
      obcUS.setMaxResults(1);
      User user = (User) obcUS.uniqueResult();
      if (user != null && partner.getSalesRepresentative() != null) {
        info.addResult("inpsalesrepId", user.getId().toString());
      }
    }

    /*
     * if (tld != null && tld.length > 0) { info.addSelect("inpsalesrepId"); for (int i = 0; i <
     * tld.length; i++) { info.addSelectResult(tld[i].getField("id"), tld[i].getField("name"),
     * tld[i].getField("id").equalsIgnoreCase(strUserRep)); }
     * 
     * info.endSelect();
     * 
     * } else { info.addResult("inpsalesrepId", ""); }
     */
    ...
    info.addResult("inpisdiscountprinted",
        SSFCOrderBPartnerData.getIsDicountPrinted(this, strBPartner));
    ...
    if (!(strwindow.equals(rtvendor) || strwindow.equals(rfcustomer))) {
      if ((!strBPartner.equals(""))
        ...
        // If the Business Partner is blocked for this document, show an information
        // message.
        if (message.length() > 0) {
          message.append("<br>");
        }
