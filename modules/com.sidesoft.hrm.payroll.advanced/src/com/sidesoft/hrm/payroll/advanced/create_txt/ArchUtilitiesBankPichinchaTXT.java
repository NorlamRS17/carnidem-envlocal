  ...
  public void doExecute(ProcessBundle bundle) throws Exception {
    ...
    try {
      ...
      final String ssprcategoryacctid = (String) bundle.getParams().get("ssprCategoryAcctId");
			final String CostCenter = (String) bundle.getParams().get("cCostcenterId");
      // Get the Payroll Ticket data
      // Obtener los datos de la Boleta de Nomina
			ArchUtilitiesBankPichinchaTXTData data[] = ArchUtilitiesBankPichinchaTXTData.select(conn, cyearid,
					ssfibanktransferid, ssprcategoryacctid, CostCenter);
          ...
          for (ArchUtilitiesBankPichinchaTXTData archUTransfData : data) {
            if (Double.valueOf(archUTransfData.valor) != 0) {
              ...
              out.write(removeAcute(archUTransfData.nomclie));
							if (ssfibanktransferid == null || ssfibanktransferid.isEmpty()) {
								out.write("\t");
								out.write(archUTransfData.codigo);
								out.write("\t");
								out.write(archUTransfData.transfer);
							}
              // out.write("\t");
