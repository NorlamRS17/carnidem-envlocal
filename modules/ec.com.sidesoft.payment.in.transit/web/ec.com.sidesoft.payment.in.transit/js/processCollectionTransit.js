isc.defineClass('OBEXAPP_ProcessCollectionTransitPopup', isc.OBPopup);
isc.OBEXAPP_ProcessCollectionTransitPopup.addProperties({
  width: 320,
  height: 200,
  title: null,
  ...
  initWidget: function () {
    ...
    this.okButton = isc.OBFormButton.create({
      ...
      popup: this,
      action: function () {
        var context = this.popup.params.button.contextView;
        var callback = function (rpcResponse, data, rpcRequest) {
          console.log(data);
          if (data.status == 'OK') {
            context.viewGrid.refreshGrid();
            isc.say('Cobros procesados con éxito');
          } else {
...
OB.OBEXAPP.SSPITRA = {
  execute: function (params, view) {
    ...
    isc.OBEXAPP_ProcessCollectionTransitPopup.create({
      view: view,
      params: params,
      actionHandler: 'ec.com.sidesoft.payment.in.transit.ad_actions.ProcessCollectionTransitActionHandler',
      rows: rows,
    }).show();

  },

  processCollectionTransit: function (params, view) {
    OB.OBEXAPP.SSPITRA.execute(params, view);
  },
};
