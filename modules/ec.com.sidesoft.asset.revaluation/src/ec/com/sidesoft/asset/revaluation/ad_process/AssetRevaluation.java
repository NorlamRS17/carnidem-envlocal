...
public class AssetRevaluation extends DalBaseProcess {
  private static final Logger log4j = Logger.getLogger(AssetRevaluation.class);
  ...
  private SsarvValueChange revaluedAsset;
  ...
  public void doExecute(ProcessBundle bundle) throws Exception {
    ...
    try {
      ...
      Asset asset = (Asset) OBDal.getInstance().getProxy(Asset.ENTITY_NAME, strAssetId);

      OBCriteria<SeactAssetSetup> assetSetup = OBDal.getInstance()
          .createCriteria(SeactAssetSetup.class);
      assetSetup.add(Restrictions.eq(SeactAssetSetup.PROPERTY_DEPRECIATION, true));
      if (assetSetup.list().size() == 0) {
        throw new RuntimeException("@SEACT_DoctypeNotDefined@");
      }
      documentType = assetSetup.list().get(0).getDoctype();

      /*
       * RARC
       */
      final String changeDate = (String) bundle.getParams().get("changedate");
      final String startDate = (String) bundle.getParams().get("startdate");
      final String newAssetValue = (String) bundle.getParams().get("newassetvalue");
      final String usefulLife = (String) bundle.getParams().get("usefullife");
      final String description = (String) bundle.getParams().get("description");

      asset = getRevaluedAsset(asset, changeDate, startDate, newAssetValue, usefulLife,
          description);
      ...
      log4j.error("Exception found in AssetRevaluation process: ", e);
  ...
  private Asset getRevaluedAsset(Asset asset, String changeDate, String startDate,
      String newAssetValue, String usefulLife, String description) {
    ...
    try {
      ...
      OBCriteria<TaxRate> taxRateList = OBDal.getInstance().createCriteria(TaxRate.class);
      taxRateList.add(Restrictions.eq(TaxRate.PROPERTY_TAXCATEGORY, asset.getSsataxCTaxcategory()));
      BigDecimal taxAmt = new BigDecimal(0);
      BigDecimal totalPurchaseAmt;
      if (taxRateList.count() != 0) {
        TaxRate taxRate = taxRateList.list().get(0);
        if (taxRate.getRate().compareTo(BigDecimal.ZERO) != 0) {
          BigDecimal tax = taxRate.getRate().divide(new BigDecimal(100));
          taxAmt = bNewAssetValue.multiply(tax);
        }
      }
      totalPurchaseAmt = bNewAssetValue.add(taxAmt);

      // Creamos la linea de cambio de valor
      revaluedAsset = OBProvider.getInstance().get(SsarvValueChange.class);
      revaluedAsset.setOrganization(asset.getOrganization());
      revaluedAsset.setClient(asset.getClient());
      revaluedAsset.setAsset(asset);

      // Valores historicos
      revaluedAsset.setHVDatepurchased(asset.getPurchaseDate());
      revaluedAsset.setHVAmortizationtype(asset.getDepreciationType());
      revaluedAsset.setHVAmortizationcalctype(asset.getCalculateType());
      revaluedAsset.setHVAmortizationstartdate(asset.getDepreciationStartDate());
      revaluedAsset.setHVAmortizationenddate(asset.getDepreciationEndDate());
      revaluedAsset.setHVAssetschedule(asset.getAmortize());
      revaluedAsset.setHVUselifemonths(asset.getUsableLifeMonths());
      revaluedAsset.setHVAssetvalueamt(asset.getAssetValue());
      revaluedAsset.setHVResidualassetvalueamt(asset.getResidualAssetValue());
      revaluedAsset.setHVAmortizationvalueamt(asset.getDepreciationAmt());
      revaluedAsset.setHVDepreciatedpreviousamt(asset.getPreviouslyDepreciatedAmt());
      revaluedAsset.setHVNetWorth(asset.getSsamNetWorth());
      revaluedAsset.setHVValueInBooks(asset.getSsamValueInBooks());
      revaluedAsset.setHVTaxcategory(asset.getSsataxCTaxcategory());
      revaluedAsset.setHVTaxamt(asset.getSsataxTaxamt());
      revaluedAsset.setHVTotalpurchaseamt(asset.getSsataxTotalpurchaseamt());

      // Valores nuevos
      revaluedAsset.setNVChangedate(dChangeDate);
      revaluedAsset.setNVDescription(description);
      revaluedAsset.setNVDatepurchased(asset.getPurchaseDate());
      revaluedAsset.setNVAmortizationtype(asset.getDepreciationType());
      revaluedAsset.setNVAmortizationcalctype(asset.getCalculateType());
      revaluedAsset.setNVAmortizationstartdate(dStartDate);
      revaluedAsset.setNVAmortizationenddate(asset.getDepreciationEndDate());
      revaluedAsset.setNVAssetschedule(asset.getAmortize());
      revaluedAsset.setNVUselifemonths(lUsefulLife);
      revaluedAsset.setNVAssetvalueamt(bNewAssetValue);

      // Calcular Valor Residual
      BigDecimal residual = new BigDecimal(0);
      residual = bNewAssetValue.multiply(new BigDecimal(0.1));
      revaluedAsset.setNVResidualassetvalueamt(residual);

      // Calcular Valor a Depreciar
      BigDecimal depreciated = bNewAssetValue.subtract(residual);
      revaluedAsset.setNVAmortizationvalueamt(depreciated);

      revaluedAsset.setNVTaxcategory(asset.getSsataxCTaxcategory());
      revaluedAsset.setNVTaxamt(taxAmt);
      revaluedAsset.setNVTotalpurchaseamt(totalPurchaseAmt);

      // Actualizar valores del activo
      asset.setDepreciationStartDate(dStartDate);
      asset.setAssetValue(bNewAssetValue);
      asset.setUsableLifeMonths(lUsefulLife);
      asset.setResidualAssetValue(residual);
      asset.setDepreciationAmt(depreciated);
      asset.setSsataxTaxamt(taxAmt);
      asset.setSsataxTotalpurchaseamt(totalPurchaseAmt);
    } catch (Exception e) {
      System.out.println("getRevaluedAsset: " + e.getMessage());
      throw new RuntimeException("getRevaluedAsset: " + e.getMessage());
    }
...
/*
    ...
    try {
      ...
      msg.setMessage(OBMessageUtils
          .messageBD("Error al obtener información de activos hijos. " + e.getMessage()));
      return msg;
    ...
    if (booChangeValue) {
      ...
      try {
        ...
        BigDecimal DepreciatedAmt = assetparent.getDepreciatedValue()
            .add(assetparent.getPreviouslyDepreciatedAmt());
        asset2.setPurchaseDate(assetparent.getPurchaseDate());
        asset2.setPreviouslyDepreciatedAmt(DepreciatedAmt);
        BigDecimal ValueInBooks = asset2.getAssetValue().subtract(asset2.getDepreciatedValue())
            .subtract(asset2.getPreviouslyDepreciatedAmt());
        asset2.setSsamValueInBooks(ValueInBooks);
        ...
        msg.setTitle(OBMessageUtils.messageBD("Error"));
        msg.setMessage(OBMessageUtils.messageBD(
            "Error al actualizar fecha de compra, depreciación anterior y valor en libros."));
        return msg;
      }
    }
...
*/
     ...
     * RARC
     */
    String strTypeofchange = asset.getSsamTypeofchange();
    Date startDate = null;
    try {
      startDate = asset.getDepreciationStartDate();
      Integer intContador = getAmortizationProcessed(asset.getId(), startDate);
      if (intContador > 0) {
        msg.setType("Error");
        msg.setTitle(OBMessageUtils.messageBD("Error"));
        msg.setMessage(OBMessageUtils.messageBD("El activo '" + asset.getName()
            + "' no puede depreciarse en un periodo ya procesado."));
        return msg;
      }
      
      String strRespuesta = deleteParentLines(asset.getId(), startDate);
      if (!strRespuesta.equals("OK")) {
        msg.setType("Error");
        msg.setTitle(OBMessageUtils.messageBD("Error"));
        msg.setMessage(OBMessageUtils
            .messageBD("Error al eliminar líneas de amortización del activo."));
        return msg;
      }
    } catch (Exception e) {
      System.out.println("RARC: "+e.getMessage());
    }
    ...
    OBDal.getInstance().save(revaluedAsset);
  ...
  private Amortization getAmortization(Organization org, Date startDate, Date endDate,
    ...
    if (amortizationList.size() == 0) {
      ...
      throw new OBException("Existe más de una cabecera de depreciación con fechas desde: "
          + (startDate == null ? " null " : startDate.toString()) + " hasta: "
          + (endDate == null ? " null " : endDate.toString()) + " para la organización: "
  ...
  private AmortizationLine getAmortizationLine(Asset asset, Date startDate, Date endDate)
    ...
    if (amortizationLineList.size() == 0) {
      ...
      throw new OBException("Existe más de una línea de depreciación con fechas desde: "
          + (startDate == null ? " null " : startDate.toString()) + " hasta: "
          + (endDate == null ? " null " : endDate.toString()) + " para el activo: "
  ...
  private Amortization createNewAmortization(Organization organization, String name,
    ...
    am.setDocumentType(documentType);
