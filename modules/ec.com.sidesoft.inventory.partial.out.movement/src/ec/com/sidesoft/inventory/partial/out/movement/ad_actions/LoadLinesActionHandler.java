...
public class LoadLinesActionHandler extends BaseActionHandler {
  ...
  protected JSONObject execute(Map<String, Object> parameters, String data) {
    ...
    try {
      ...
      for (int i = 0; i < rows.length(); i++) {
        String result = loadLines(rows.getString(i));
        if (!result.equals("OK")) {
  ...
  private String loadLines(String recordId) {
    String result = null;
    ConnectionProvider conn = new DalConnectionProvider(false);
    try {
      String sql = "SELECT ssipotm_loadlines(?) AS result";
