...
public class EcsbaAccLoad extends DalBaseProcess {
  ...
  protected void doExecute(ProcessBundle bundle) throws Exception {
    ...
    try {
      ...
      if (accountId != null && !accountId.isEmpty()) {
        ...
        treeCriteria.add(Restrictions.eq(TreeNode.PROPERTY_REPORTSET, accountId));
        treeCriteria.add(Restrictions.eq(TreeNode.PROPERTY_TREE, accountTree));
        ...
        if (childNodes.isEmpty()) {
          ...
          message.setMessage("La cuenta seleccionada no tiene hijos inmediatos.");
        } else {
          for (TreeNode node : childNodes) {
            // El TreeNode guarda el ID como string en la propiedad 'node'
            ...
            if (childAccount != null && childAccount.isActive()
                && Boolean.TRUE.equals(childAccount.isSsavIsmodule())) {
              insertAccount(childAccount, breakdownBalance);
              counter++;
            }
          }
          message.setMessage("Se agregaron " + counter + " cuentas hijas.");
        }
      } else {
        // --- CASO 2: VACÍO (Buscar por ssavIsmodule) ---
  ...
  private void insertAccount(ElementValue elementValue, EcsbaBreakdownBalance parentRecord) {
    ...
    check
        .add(Restrictions.eq(EcsbaAccountingAccounts.PROPERTY_ECSBABREAKDOWNBALANCE, parentRecord)); // Importante
                                                                                                     // chequeo
    // check.setMaxResults(1);
