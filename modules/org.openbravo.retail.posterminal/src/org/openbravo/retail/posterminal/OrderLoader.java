...
public class OrderLoader extends POSDataSynchronizationProcess
  ...
  public JSONObject saveRecord(JSONObject jsonorder) throws Exception {
    ...
    try {
      ...
      try {
        ...
        //     && Math.abs(jsonorder.getDouble("payment")) < Math
        //         .abs(new Double(jsonorder.getDouble("gross")));

        BigDecimal grossRound = BigDecimal.valueOf(jsonorder.getDouble("gross"));
        grossRound = grossRound.setScale(2, BigDecimal.ROUND_HALF_UP);
        
        BigDecimal grossRound1 = BigDecimal.valueOf(jsonorder.getDouble("payment"));
        grossRound1 = grossRound1.setScale(2, BigDecimal.ROUND_HALF_UP);
        
        wasPaidOnCredit = !isQuotation && !isDeleted && !notpaidLayaway
            &&   Math.abs( grossRound1.doubleValue() ) < 
                 Math.abs( grossRound.doubleValue()  );         

        if (jsonorder.has("oBPOSNotInvoiceOnCashUp")
            && jsonorder.getBoolean("oBPOSNotInvoiceOnCashUp")) {
          createInvoice = false;
        } else {
          createInvoice = wasPaidOnCredit || (!isQuotation && !notpaidLayaway
        ...
        if (createShipment) {
          ...
          if (locatorList.isEmpty()) {
            throw new OBException(
                Utility.messageBD(new DalConnectionProvider(false), "OBPOS_WarehouseNotStorageBin",
	...
	customIsLayaway = (customIsLayaway) || (customOrderType == 2);
        if (createShipment && PreferenceToValidateStock() && !customIsLayaway) {
          ...
          String strTypeTrans = "ND";

          if (jsonorderrev.length() > 0) {

            JSONObject jsonPayment = jsonorderrev.getJSONObject(0);

            strTypeTrans = jsonPayment.getString("isReturnOrder").equals("true") ? "DEV" : "VNT";
          }
          ...
          CallableStatement updateStockStatement = cp.getConnection()
              .prepareCall("{call SSPSTPV_UPDATE_INVENTORY (?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"); // ORIGINAL
                                                                                             // -
                                                                                             // M_UPDATE_INVENTORY
          try {
            // Stock manipulation
            handleStock(shipment, updateStockStatement, strTypeTrans);
          } finally {
      ...
      ImportEntryManager.getInstance().reportStats("orderLoader",
  ...
  private void createInvoiceLine(Invoice invoice, Order order, JSONObject jsonorder,
    ...
    boolean deliveredQtyEqualsToMovementQty = movementQtyTotal
        .compareTo(lineReferences.get(numIter).getOrderedQuantity()) == 0;
    ...
    line.setDescription(orderlines.getJSONObject(numIter).has("description")
        ? orderlines.getJSONObject(numIter).getString("description")
        : "");
    ...
    if (ratio.compareTo(BigDecimal.ONE) != 0) {
      ...
      line.setLineNetAmount(BigDecimal.valueOf(orderlines.getJSONObject(numIter).getDouble("net"))
          .setScale(pricePrecision, RoundingMode.HALF_UP));
      line.setGrossAmount(lineReferences.get(numIter).getLineGrossAmount().setScale(pricePrecision,
          RoundingMode.HALF_UP));
    }

    line.setInvoicedQuantity(qty);
    lineReferences.get(numIter)
        .setInvoicedQuantity((lineReferences.get(numIter).getInvoicedQuantity() != null
            ? lineReferences.get(numIter).getInvoicedQuantity().add(qty)
            : qty));
    ...
    while (itKeys.hasNext()) {
      ...
      if (ratio.compareTo(BigDecimal.ONE) != 0) {
        ...
        if (numLines > actualLine || (numLines == actualLine && !deliveredQtyEqualsToMovementQty)) {
          ...
          for (InvoiceLineTax ilt : invoice.getInvoiceLineTaxList()) {
            if (ilt.getInvoiceLine().getSalesOrderLine() != null && ilt.getInvoiceLine()
                .getSalesOrderLine().getId() == lineReferences.get(numIter).getId()
      ...
      invoicelinetax.setId(OBMOBCUtils
          .getUUIDbyString(line.getSalesOrderLine().getId() + lineNo + (long) ((ind + 1) * 10)));
  ...
  private void createInvoiceLines(Invoice invoice, Order order, JSONObject jsonorder,
    ...
    int lineNo = 0;
    for (int i = 0; i < orderlines.length(); i++) {
      final OBCriteria<ShipmentInOutLine> iolCriteria = OBDal.getInstance()
          .createCriteria(ShipmentInOutLine.class);
      iolCriteria
          .add(Restrictions.eq(ShipmentInOutLine.PROPERTY_SALESORDERLINE, lineReferences.get(i)));
  ...
  protected void createInvoice(Invoice invoice, Order order, JSONObject jsonorder)
    ...
    invoice.setOutstandingAmount((BigDecimal.valueOf(jsonorder.getDouble("gross")))
        .setScale(pricePrecision, RoundingMode.HALF_UP));
    invoice.setDueAmount((BigDecimal.valueOf(jsonorder.getDouble("gross"))).setScale(pricePrecision,
        RoundingMode.HALF_UP));
    ...
    try {
      ...
      for (i = 0; i < payments.length(); i++) {
        ...
        if (payment.has("isPrePayment") && payment.getBoolean("isPrePayment")) {
          total = total.subtract(BigDecimal.valueOf(payment.getDouble("origAmount"))
              .setScale(pricePrecision, RoundingMode.HALF_UP));
        }
      }
      ...
      // SE VERIFICA QUE EL TERCERO NO SEA CONSUMIDOR FINAL PARA HACER EL UPDATE AL CREDITO USADO
      // SE BUSCA EN LAS ORGANIZACIONES TODOS LOS CLIENTES ANONIMOS
      // SI EL TERCERO ESTA DENTRO DE LOS CLIENTES ANONIMOS DE LAS ORGANIZACION
      // ENTONCES NO SE ACTUALIZA EL CREDITO USADO 
      OBCriteria<Organization> finalConsumer = OBDal.getInstance()
          .createCriteria(Organization.class);
      finalConsumer.add(Restrictions.eq(Organization.PROPERTY_ACTIVE, true));
      finalConsumer.add(Restrictions.eq(Organization.PROPERTY_OBRETCOCBPARTNER , invoice.getBusinessPartner() ));   
      
      if(finalConsumer.list().size() == 0) {
        // Same currency, no conversion required
        invoice.getBusinessPartner()
            .setCreditUsed(invoice.getBusinessPartner().getCreditUsed().add(total)); 
      }         
      OBDal.getInstance().flush();
    } finally {
      OBContext.restorePreviousMode();
  ...
  protected void handleStock(ShipmentInOut shipment, CallableStatement updateStockStatement,
    ...
    for (ShipmentInOutLine line : shipment.getMaterialMgmtShipmentInOutLineList()) {
      if (line.getProduct().getProductType().equals("I") && line.getProduct().isStocked()) {
		...
		//Ticket 6225 - ELIMINACIÓN DE BANDERA POR LOGS LLENOS
        //System.out.print(strTypeTrx);

        updateInventory(transaction, updateStockStatement, strTypeTrx);
  ...
  protected void updateInventory(MaterialTransaction transaction,
      CallableStatement updateStockStatement, String strTrans) {
    try {
      // client
      ...
      updateStockStatement.setString(14, strTrans != null ? strTrans : null);
  ...
  public JSONObject handlePayments(JSONObject jsonorder, Order order, Invoice invoice,
    ...
    BigDecimal gross = BigDecimal.valueOf(jsonorder.getDouble("gross")),
        paymentAmt = BigDecimal.valueOf(jsonorder.getDouble("payment"));
    ...
    if (invoice != null && invoice.getGrandTotalAmount().compareTo(BigDecimal.ZERO) != 0) {
      ...
      if (createInvoice) {
        ...
        paymentScheduleInvoice
            .setOutstandingAmount(gross.setScale(pricePrecision, RoundingMode.HALF_UP));

        if (wasPaidOnCredit) {
          OBCriteria<PaymentTermLine> lineCriteria = OBDal.getInstance()
          ...
          if (termLineList.size() > 0) {
            ...
            for (PaymentTermLine paymentTermLine : termLineList) {
              ...
              if (paymentTermLine.isExcludeTax()) {
                amount = (order.getSummedLineAmount()
                    .multiply(paymentTermLine.getPercentageDue().divide(new BigDecimal("100"))))
                        .setScale(pricePrecision, RoundingMode.HALF_UP);
        ...
        if (gross.signum() >= 0) {
          ...
          paymentScheduleInvoice.setOutstandingAmount(
              amountPaidWithCredit.setScale(pricePrecision, RoundingMode.HALF_UP));
        } else {
          paymentScheduleInvoice.setPaidAmount(paymentScheduleInvoice.getAmount()
    ...
    boolean checkPaidOnCreditChecked = (jsonorder.has("paidOnCredit")
        && jsonorder.getBoolean("paidOnCredit"));
    if (invoice != null
      ...
      invoice.setDaysTillDue(
          FIN_Utility.getDaysToDue(paymentScheduleInvoice == null ? invoice.getInvoiceDate()
              : paymentScheduleInvoice.getDueDate()));
      if (paymentScheduleInvoice != null
        ...
        if (paymentScheduleInvoice.getAmount().signum() >= 0) {
          if (paymentScheduleInvoice.getAmount().compareTo(paymentAmt) >= 0) {
            paymentScheduleInvoice
                .setOutstandingAmount(paymentScheduleInvoice.getAmount().subtract(paymentAmt));
          ...
          if (paymentScheduleInvoice.getAmount().compareTo(paymentAmt) <= 0) {
            paymentScheduleInvoice
                .setOutstandingAmount(paymentScheduleInvoice.getAmount().add(paymentAmt));
  ...
  private void setRemainingPayment(Order order, Invoice invoice,
    ...
    for (FIN_PaymentScheduleDetail pSched : pScheduleDetails) {
      if (pSched.getPaymentDetails() == null) {
        ...
        if (paymentScheduleInvoice != null
            && paymentScheduleInvoice
                .getFINPaymentScheduleDetailInvoicePaymentScheduleList() != null
    ...
    if (usedCredit || diffPaid.compareTo(BigDecimal.ZERO) != 0) {
      ...
      if (invoice != null) {
        ...
        Collections.sort(paymentScheduleInvoiceList, new Comparator<Object>() {
          @Override
          public int compare(Object o1, Object o2) {
            return ((FIN_PaymentSchedule) o1).getDueDate()
                .compareTo(((FIN_PaymentSchedule) o2).getDueDate());
          }
        });
        ...
        for (FIN_PaymentSchedule paymentScheduleInv : paymentScheduleInvoiceList) {
          ...
          if (outstandingPaidAmount.signum() >= 0) {
            if (outstandingPaidAmount.compareTo(paymentScheduleInv.getOutstandingAmount()) >= 0) {
              ...
              outstandingPaidAmount = outstandingPaidAmount
                  .subtract(paymentScheduleInv.getOutstandingAmount());
            } else {
        ...
        FIN_PaymentScheduleDetail paymentScheduleDetail = OBProvider.getInstance()
  ...
  protected void processPayments(FIN_PaymentSchedule paymentSchedule,
    ...
    try {
      ...
      boolean hasReceiptChange = jsonorder.getDouble("change") > 0,
          isChangePayment = payment.optBoolean("isChange", false),
          hasPaymentScheduleInvoice = false;
      boolean checkPaidOnCreditChecked = (jsonorder.has("paidOnCredit")
          && jsonorder.getBoolean("paidOnCredit"));
      int pricePrecision = order.getCurrency().getObposPosprecision() == null
          ? order.getCurrency().getPricePrecision().intValue()
      ...
      if (writeoffAmt.signum() == 1) {
        ...
        if (totalIsNegative) {
          ...
          amount = amount.subtract(writeoffAmt.abs()).setScale(pricePrecision,
              RoundingMode.HALF_UP);
        }
      ...
      BigDecimal gross = BigDecimal.valueOf(jsonorder.getDouble("gross"));
		gross = gross.setScale(2, BigDecimal.ROUND_HALF_UP);
      ...
      if (invoice != null && invoice.getGrandTotalAmount().compareTo(BigDecimal.ZERO) != 0
        ...
        if (paymentScheduleInvoiceList != null && paymentScheduleInvoiceList.size() > 0) {
          ...
          for (FIN_PaymentSchedule paymentScheduleInv : paymentScheduleInvoiceList) {
            ...
            if (outstandingPaidAmount.signum() >= 0) {
              ...
              if (paymentScheduleInv.getOutstandingAmount().signum() >= 0) {
                ...
                if (outstandingPaidAmount
                    .compareTo(paymentScheduleInv.getOutstandingAmount()) <= 0) {
                  psdAmount = paymentScheduleInv.getOutstandingAmount();
                  outstandingPaidAmount = outstandingPaidAmount
                      .subtract(paymentScheduleInv.getOutstandingAmount());
                } else {
            ...
            paymentScheduleInv.getFINPaymentScheduleDetailInvoicePaymentScheduleList()
                .add(paymentScheduleDetail);
            paymentScheduleDetail.setInvoicePaymentSchedule(paymentScheduleInv);
          ...
          Fin_OrigPaymentSchedule origPaymentSchedule = OBProvider.getInstance()
              .get(Fin_OrigPaymentSchedule.class);
          ...
          FIN_OrigPaymentScheduleDetail origDetail = OBProvider.getInstance()
              .get(FIN_OrigPaymentScheduleDetail.class);
      ...
      if (writeoffAmt.signum() == 1
        ...
        if (totalIsNegative) {
          FIN_AddPayment.saveGLItem(finPayment, writeoffAmt.negate(),
