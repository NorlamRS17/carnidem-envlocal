...
enyo.kind({
  ...
  updateCreditSalesAction: function () {
    ...
    if (visible) {
      if(this.receipt.get('bp').get('searchKey') === '9999999999'){
        this.$.creditsalesaction.hide();
      }else{
        var boton =  this.$.creditsalesaction;
        var serverCall = new OB.DS.Process('ec.com.sidesoft.retail.credit.PendingAmt');
        serverCall.exec({
          bpId: this.receipt.get('bp').get('id')
        }, function (data, message) {
          if(data && data.pendingAmt){
