package ec.com.sidesoft.ws.paymentscreate.webservices;
...
public class Portfolio implements WebService {
  ...
  private JsonArray getPortfolio(String documentNo, String taxId) {
    ...
    try {
      String sql = "SELECT i.c_invoice_id, i.documentno, i.dateinvoiced::DATE dateinvoiced, i.grandtotal, i.outstandingamt i_outstandingamt, i.em_eei_urlxml url_xml, i.em_eei_urlride url_ride "
          + "   , bp.c_bpartner_id, bp.taxid, bp.name bp_name"
          + "   , pt.c_paymentterm_id, pt.name pt_name"
          + "   , ps.fin_payment_schedule_id, ps.em_ssdnid_cutoff_date::DATE cutoff_date, ps.amount, ps.paidamt, ps.outstandingamt ps_outstandingamt "
          + "   , COALESCE(ps.em_ssdnid_interestlatepayment,0) interestlatepayment, COALESCE(ps.em_ssdnid_collectionexpenses,0) collectionexpenses "
          + "   , CASE " + "           WHEN ps.outstandingamt = 0 THEN "
          + "                   CASE "
          + "                           WHEN MAX(p.paymentdate)::DATE - ps.em_ssdnid_cutoff_date::DATE <= 0 THEN 0 "
          + "                           ELSE MAX(p.paymentdate)::DATE - ps.em_ssdnid_cutoff_date::DATE "
          + "                   END "
