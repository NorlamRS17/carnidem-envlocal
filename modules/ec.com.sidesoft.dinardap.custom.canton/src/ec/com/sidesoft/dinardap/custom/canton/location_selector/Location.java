 ...
 ************************************************************************
 */
package ec.com.sidesoft.dinardap.custom.canton.location_selector;
...
public class Location extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strWindow = vars.getSessionValue("Location.inpwindowId");
      printPageF3(response, vars, strcRegionId, strWindow);
    } else
      pageError(response);
  }
  ...
  private LocationSearchData getEditVariables(VariablesSecureApp vars) {
    ...
    data.emSdinccCantonId = vars.getStringParameter("inpEmSdinccCantonId");
    return data;
  }
  ...
  private void printPageKey(HttpServletResponse response, VariablesSecureApp vars,
    ...
    XmlDocument xmlDocument = xmlEngine
        .readXmlTemplate(
            "ec/com/sidesoft/dinardap/custom/canton/location_selector/SearchUniqueKeyResponse")
        .createXmlDocument();
  ...
  private void printPageFS(HttpServletResponse response, VariablesSecureApp vars)
    ...
    XmlDocument xmlDocument = xmlEngine
        .readXmlTemplate("ec/com/sidesoft/dinardap/custom/canton/location_selector/Location_FS")
        .createXmlDocument();
  ...
  private void printPageSheet(HttpServletResponse response, VariablesSecureApp vars,
    ...
    if (strcLocationId.equals("")) {
      xmlDocument = xmlEngine
          .readXmlTemplate("ec/com/sidesoft/dinardap/custom/canton/location_selector/Location_F1")
          .createXmlDocument();
    ...
    try {
      // C21E8BC209514656888CDE02EE4BB909 id de la regla de validacion --> Canton By Region
      ComboTableData comboTableData = new ComboTableData(vars, this, "TABLE", "Secpm_Canton",
          "4A056FBCED6A47CBA71671855264514D", "C21E8BC209514656888CDE02EE4BB909",
          Utility.getReferenceableOrg(vars, vars.getOrg()),
          Utility.getContext(this, vars, "#User_Client", strWindow), 0);
      Utility.fillSQLParameters(this, vars, data[0], comboTableData, strWindow,
          data[0].emSdinccCantonId);
      xmlDocument.setData("reportCanton", "liststructure", comboTableData.select(false));
      comboTableData = null;
    } catch (Exception ex) {
      throw new ServletException(ex);
    }
  ...
  private void printPageF2(HttpServletResponse response, VariablesSecureApp vars,
    ...
    XmlDocument xmlDocument = xmlEngine
        .readXmlTemplate("ec/com/sidesoft/dinardap/custom/canton/location_selector/Location_F2")
        .createXmlDocument();
    // Getcountry; default
    try {
      ComboTableData comboTableData = new ComboTableData(vars, this, "TABLEDIR", "C_Region_ID", "",
  ...
  private void printPageF3(HttpServletResponse response, VariablesSecureApp vars,
      String strcRegionId, String strWindow) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("Output: F3 Locations seeker");
    XmlDocument xmlDocument = xmlEngine
        .readXmlTemplate("ec/com/sidesoft/dinardap/custom/canton/location_selector/Location_F3")
        .createXmlDocument();
    // Getcountry; default
    try {
      // C21E8BC209514656888CDE02EE4BB909 id de la regla de validacion --> Canton By Region
      ...
      Utility.fillSQLParameters(this, vars, null, comboTableData, strWindow, "");
      xmlDocument.setParameter("array",
          Utility.arrayEntradaSimple("cantons", comboTableData.select(false)));
      comboTableData = null;
    } catch (Exception ex) {
      throw new ServletException(ex);
    }
