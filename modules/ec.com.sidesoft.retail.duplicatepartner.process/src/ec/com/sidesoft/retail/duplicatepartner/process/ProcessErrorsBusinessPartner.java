  ...
  protected void doExecute(ProcessBundle bundle) throws Exception {
    ...
    try {
      ...
      for (OBPOSErrors error : listErrors) {
        ...
        String idemp = "";
        String idmain=json.getString("id");
   ...
   // ********************************************************************
      // PROCESO DE UPDATE PEDIDOS CON estado_envio Y estado_factura AL 100%
      // ********************************************************************   
      OBCriteria<OBPOSErrors> queryListErrorsData = OBDal.getInstance().createCriteria(
    		  OBPOSErrors.class);
      	queryListErrorsOutDatedData.add(Restrictions.eq(OBPOSErrors.PROPERTY_ORDERSTATUS, "N"));
        queryListErrorsOutDatedData.add(Restrictions.eq(OBPOSErrors.PROPERTY_TYPEOFDATA, "Order"));
        List<OBPOSErrors> listErrorsData = queryListErrorsData.list();
        Collections.sort(listErrorsData, new ErrorComparator());
          for (OBPOSErrors errorOutDatedData : listErrorsData) {             
        	  JSONObject json = new JSONObject(errorOutDatedData.getJsoninfo());
              String idOrder=json.getString("id");
              Order order = OBDal.getInstance().get(Order.class, idOrder);
              if(order.getDeliveryStatus().equals((long) 100) &&order.getInvoiceStatus().equals((long) 100)) {
            	  errorOutDatedData.setOrderstatus("Y");
            	  OBDal.getInstance().save(errorOutDatedData);
              }
           }   
      // ********************************************************************
...
//      logger.logln("Proceso completado satisfactoriamente. timezoneOffset");     
  ...
  public void updateAttempts(){
    ...
    String strSql = "UPDATE obpos_errors SET attempts = 0;";    
