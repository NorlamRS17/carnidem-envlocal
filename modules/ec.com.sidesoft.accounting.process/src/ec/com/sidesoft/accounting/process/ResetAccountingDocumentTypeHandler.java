  ...
  protected JSONObject doExecute(Map<String, Object> parameters, String content) {
    ...
    try {
	  ...
	  PreparedStatement st = null;
		...
		try {
			String sql = "select cp.name, cp.openclose, \n"
					+ "	CASE\n"
					+ "         WHEN (max(cpc.periodstatus) = min(cpc.periodstatus) AND min(cpc.periodstatus) = 'O') THEN 'O'\n"
					+ "         WHEN (max(cpc.periodstatus) = min(cpc.periodstatus) AND min(cpc.periodstatus) ='C') THEN 'C'\n"
					+ "         WHEN (max(cpc.periodstatus) = min(cpc.periodstatus) AND min(cpc.periodstatus) ='P') THEN 'P'\n"
					+ "         WHEN (max(cpc.periodstatus) = min(cpc.periodstatus) AND min(cpc.periodstatus) = 'N') THEN 'N'\n"
					+ "		ELSE 'M'\n"
					+ "	END as status\n"
					+ "from c_period cp\n"
					+ "left join c_periodcontrol cpc on cpc.c_period_id = cp.c_period_id\n"
					+ "where TO_DATE(cp.startdate) >= TO_DATE(?, 'DD-MM-YYYY')\n"
					+ " AND TO_DATE(cp.enddate::text, 'YYYY-MM-DD') <= (DATE_TRUNC('month', TO_DATE(?, 'DD-MM-YYYY')) + INTERVAL '1 month - 1 day')::date \n"
					+ " GROUP BY cp.name, cp.openclose";
			...
			while (rs.next()) {
				...
				if (status.equals("M") || status.equals("C") || status.equals("P")){
					throw new OBException("No se puede procesar registros que se encuentren en un periodo Cerrado");
				}
			}
