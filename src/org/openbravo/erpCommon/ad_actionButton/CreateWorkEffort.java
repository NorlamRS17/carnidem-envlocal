        ...
        if (pplanList.isEmpty()) {
          ...
          if (wrOp.isCreateStandards()) {
        	  ...
        	  if (wrOp.getEstimatedTime() != null && wrOp.getQuantity() != null
        	    ...
        	    // Obtener la precisión de costeo (CostingPrecision)
        	    int vRoundAmount = 6; // Valor por defecto

        	    try {
        	      OBCriteria<Organization> orgCrit = OBDal.getInstance().createCriteria(Organization.class);
        	      orgCrit.add(Restrictions.eq(Organization.PROPERTY_SUMMARYLEVEL, true));

        	      Organization orgEntity = (Organization) orgCrit.uniqueResult();
        	      if (orgEntity != null && orgEntity.getCurrency() != null
        	          && orgEntity.getCurrency().getCostingPrecision() != null) {
        	        vRoundAmount = orgEntity.getCurrency().getCostingPrecision().intValue();
        	      }
        	    } catch (Exception e) {
        	      log4j.debug("No se pudo obtener CostingPrecision, se usará 6 por defecto.", e);
        	    }

        	    // Cálculo con precisión dinámica
        	    estimatedTime = wrOp.getEstimatedTime()
        	        .multiply(requiredQty)
        	        .divide(wrOp.getQuantity(), vRoundAmount, RoundingMode.HALF_UP);
        	  }

        	  productionPlan.setEstimatedTime(estimatedTime);
        	} else {
