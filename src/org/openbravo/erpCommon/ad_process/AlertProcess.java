  ...
  private static AlertProcessData[] selectAlert(ConnectionProvider connectionProvider,
    ...
    try {
      ...
      while (result.next()) {
        ...
        if (strSql.contains("to_email")) {
            objectAlertProcessData.toEmail = UtilSql.getValue(result, "to_email");
        }
...
//start validation
    ...
    try {
    	  ...
    	  alertr = 	OBDal.getInstance().get(AlertRule.class, alertRule.adAlertruleId);
    ...
    if (alert != null && alert.length != 0) {
      ...
      if (alertr!= null && alertr.isSdeaIsemail()) {
        	OBContext.setAdminMode();
        	try {
                    // Getting the SMTP server parameters
                    OBCriteria<EmailServerConfiguration> mailConfigCriteriam = OBDal.getInstance()
                        .createCriteria(EmailServerConfiguration.class);
                    mailConfigCriteriam.add(Restrictions.eq(EmailServerConfiguration.PROPERTY_CLIENT, OBDal
                        .getInstance().get(Client.class, adClientId)));
                    mailConfigCriteriam.setFilterOnReadableClients(false);
                    mailConfigCriteriam.setFilterOnReadableOrganization(false);
                    final List<EmailServerConfiguration> mailConfigListm = mailConfigCriteriam.list();

                    if (mailConfigListm.size() > 0) {
                  	...
                  	// TODO: There should be a mechanism to select the desired Email server configuration
                        ...
                        EmailServerConfiguration mailConfigm = null;

                        for (EmailServerConfiguration currentOrgConfig : mailConfigListm) {
                            if (adOrgId.equals(currentOrgConfig.getOrganization().getId())) {
                              mailConfigm = currentOrgConfig;
                              break;
                            }
                          }
                          if (mailConfigm == null) {
                            for (EmailServerConfiguration zeroOrgConfig : mailConfigListm) {
                              if ("0".equals(zeroOrgConfig.getOrganization().getId())) {
                                mailConfigm = zeroOrgConfig;
                                break;
                              }
                            }
                          }
                          if (mailConfigm == null) {
                            mailConfigm = mailConfigListm.get(0);
                          }
                                                
                        if(StringUtils.isNotBlank(alert[i].toEmail)) {
                        	final String host = mailConfigm.getSmtpServer();
                            final Boolean auth = mailConfigm.isSMTPAuthentification();
                            final String username = mailConfigm.getSmtpServerAccount();
                            final String password = FormatUtilities.encryptDecrypt(
                                mailConfigm.getSmtpServerPassword(), false);
                            final String connSecurity = mailConfigm.getSmtpConnectionSecurity();
                            final int port = mailConfigm.getSmtpPort().intValue();
                            final String senderAddress = mailConfigm.getSmtpServerSenderAddress();
                            final String recipientTO = alert[i].toEmail;
                            final String recipientCC = null;
                            final String recipientBCC = null;
                            final String replyTo = null;
                            final String subject = "Por favor gestionar Verificación No. de Operación # "+alert[i].recordId;
                            final String content = alert[i].description + "\n";
