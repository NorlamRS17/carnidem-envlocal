...
public class AgingDao {
  ...
  public FieldProvider[] getOpenReceivablesAgingSchedule(ConnectionProvider connectionProvider,
    ...
    try {
      ...
      dataSR = AgingDaoData.select(connectionProvider, showDoubtfulDebt ? "Y" : "N",
          ...
          OBDateUtils.formatDate(convertToDatePluss(currentDate, strcolumn4)),
          OBDateUtils.formatDate(convertToDatePluss(currentDate, strcolumn3)),
          OBDateUtils.formatDate(convertToDatePluss(currentDate, strcolumn2)),
          OBDateUtils.formatDate(convertToDatePluss(currentDate, strcolumn1)),
        ...
        if (agingBalanceData.containsKey(strBusinessPartnerId)) {
          ...
          agingBalanceData.put(strBusinessPartnerId,
              new AgingData(strBusinessPartnerId, strBpName, BigDecimal.ZERO, BigDecimal.ZERO,
                  ...
                  BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, creditLeft,
  ...
  public FieldProvider[] getOpenReceivablesAgingScheduleDetails(
    ...
    try {
      ...
      while (dataSR.next()) {
        ...
        final String taxId = dd.taxid;
        final String dueDate = dd.duedate;
        final String days = getDays(OBDateUtils.getDate(dueDate), currentDate);
        HashMap<String, String> psData = insertData(docNo, invoiceId, invoiceDate, amount,
            ...
            dateFormat, false, doubtfulDebtAmount, dueDate, taxId, days);
  ...
  private HashMap<String, String> insertData(String documentNo, String id, Date date,
      ...
      SimpleDateFormat dateFormat, boolean credits, BigDecimal doubtfulDebt, String dueDate,
      String taxId, String days) throws ParseException {
    ...
    if (group == -4) {
      psData.put("AMOUNT14", amount.compareTo(BigDecimal.ZERO) == 0 ? null : amount.toString());
    } else if (group == -3) {
      psData.put("AMOUNT13", amount.compareTo(BigDecimal.ZERO) == 0 ? null : amount.toString());
    } else if (group == -2) {
      psData.put("AMOUNT12", amount.compareTo(BigDecimal.ZERO) == 0 ? null : amount.toString());
    } else if (group == -1) {
      psData.put("AMOUNT11", amount.compareTo(BigDecimal.ZERO) == 0 ? null : amount.toString());
    } else {
      psData.put("AMOUNT" + group,
          amount.compareTo(BigDecimal.ZERO) == 0 ? null : amount.toString());
    }
    ...
    if (!StringUtils.isEmpty(taxId)) {
      psData.put("taxId", taxId);
    } else {
      psData.put("taxId", null);
    }

    if (!StringUtils.isEmpty(dueDate)) {
      psData.put("dueDate", dueDate);
    } else {
      psData.put("dueDate", null);
    }

    if (!StringUtils.isEmpty(days)) {
      psData.put("dias", days);
    } else {
      psData.put("dias", null);
    }
  ...
  /**
   * Returns the result date of add the date range to the as Of Date field.
   */
  private Date convertToDatePluss(Date currentDate, String strcolumn) {
    Calendar cal = new GregorianCalendar();
    cal.setTime(currentDate);
    cal.add(Calendar.DATE, +Integer.parseInt(strcolumn));
    // TimeUnit.DAYS.
  ...
  private void setFieldsIntoFieldProvider(FieldProvider[] data, List<AgingData> agingBalanceData) {
    for (int i = 0; i < data.length; i++) {
      ...
      FieldProviderFactory.setField(data[i], "amount11",
          agingBalanceData.get(i).getAmount11().toString());
      FieldProviderFactory.setField(data[i], "amount2",
          agingBalanceData.get(i).getamount2().toString());
      FieldProviderFactory.setField(data[i], "amount12",
          agingBalanceData.get(i).getAmount12().toString());
      FieldProviderFactory.setField(data[i], "amount3",
          agingBalanceData.get(i).getamount3().toString());
      FieldProviderFactory.setField(data[i], "amount13",
          agingBalanceData.get(i).getAmount13().toString());
      FieldProviderFactory.setField(data[i], "amount4",
          agingBalanceData.get(i).getamount4().toString());
      FieldProviderFactory.setField(data[i], "amount14",
          agingBalanceData.get(i).getAmount14().toString());
  ...
  private String getDays(final Date duedate, Date currentDate) {
